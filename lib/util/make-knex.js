...
const { LeaseConnectionFromClient } = require('./connection-leasing');
...
for (const m of CONTEXT_METHODS) {
  KNEX_PROPERTY_DEFINITIONS[m] = {
    value: function(...args) {
      const f = this.context[m];
      if (!f) {
        // In theory, this should never happen.  But, we're making
        // the exception slightly more user-friendly just in case.
        throw new TypeError(`this.context.${m} is not a function`);
      }
