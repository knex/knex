...
class Transaction extends EventEmitter {
  constructor(client, container, config = DEFAULT_CONFIG(), outerTx = null) {
    ...
    const basePromise = this._kickThingsOff(config, container);
    ...
    this._promise = Bluebird.resolve(basePromise).then((x)=> x);
  ...
  async _kickThingsOff(config, container) {
  ...
  async _acquireConnection(config) {
    const configConnection = config && config.connection;
    return configConnection || this.client.acquireConnection();
  }
  ...
  acquireConnection(config, cb) {
    const configConnection = config && config.connection;
    return this._acquireConnection(config)
      .then((connection) => {
