...
var Transaction = (function(_EventEmitter) {
  ...
  function Transaction(client, container, config, outerTx) {
    ...
    _this._promise = _bluebird2.default.using(
      _this.acquireConnection(client, config, txid),
      function(connection) {
        var trxClient = (_this.trxClient = makeTxClient(
          _this,
          client,
          connection
        ));
        var init = client.transacting
          ? _this.savepoint(connection)
          : _this.begin(connection);
  ...
  Transaction.prototype.rollback = function rollback(conn, error) {
    ...
    return this.query(conn, 'ROLLBACK', 2, error)
      .timeout(5000)
      .catch(_bluebird2.default.TimeoutError, function() {
  ...
  Transaction.prototype.acquireConnection = function acquireConnection(
    client,
    config,
    txid
  ) {
