...
class TableBuilder {
  ...
  timestamps(param1, param2, useCamelCase) {
    const camel = useCamelCase === true;
    const method = param1 === true ? 'timestamp' : 'datetime';
    if (camel) {
      const createdAt = this[method].camelCase('created_at');
      const updatedAt = this[method].camelCase('updated_at');
      if (param2 === true) {
