...
TableCompiler.prototype.pushQuery = function(obj) {
  this.schemaCompiler.pushQuery(obj);
...
TableCompiler.prototype.create = function() {
  this.pushQuery('create table ' + this.tableName() + ' (' + this.getColumns().join(', ') + ')');
  this.alterColumns();
  this.addIndexes();
};
...
TableCompiler.prototype.addColumns = function() {
  ...
  if (columns.length > 0) {
    ...
    this.pushQuery('alter table ' + this.tableName() + ' ' + columns.join(', '));
  }
};
