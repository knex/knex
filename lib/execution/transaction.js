...
class Transaction extends EventEmitter {
  ...
  addFunctionOnAfterTransaction(cb) {
    this._afterTxCollector.push(cb);
  }

  setOnAfterTransactionCollection(collector) {
    this._afterTxCollector = collector;
  }
  ...
  _evaluateContainer(config, container) {
    return this.acquireConnection(config, (connection) =>
      this._onAcquire(container, connection)
    );
  }
...
function makeTransactor(trx, connection, trxClient) {
  ...
  transactor.setOnAfterTransactionCollection = (collector) => {
    trx.setOnAfterTransactionCollection(collector);
  }
