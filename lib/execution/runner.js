...
/**
 * @typedef {import('../query/querycompiler').ProcessResponseQueryObject<object, object>} ProcessResponseQueryObject
 * @typedef {import('../query/querycompiler').CompiledQuery} CompiledQuery
 *
 * @typedef {Object} RunnerProps
 * @property {() => any} [queryContext]
 *
 * @typedef {CompiledQuery&RunnerProps} RunnableQuery
 */
/**
 * @template [T=unknown]
 * @typedef {Object} ExecutingQueryProps
 * @property {object} response
 * @property {T} [context]
 * @property {(this: Runner, response: any) => Promise<ProcessResponseQueryObject>} [output]
 */
/**
 * @template [T=unknown]
 * @typedef {RunnableQuery & ExecutingQueryProps<T>} QueryResult
 */
...
class Runner {
  ...
  stream(optionsOrHandler, handlerOrNil) {
    ...
    const resultMapper = this.client.resultMapper(this.builder);

    let dialectContext = undefined;

    const stream = new Transform({
      objectMode: true,
      transform: (chunk, _, callback) => {
        const response = this.client.postProcessResponse(chunk, queryContext);
        resultMapper?.applyTo(response, dialectContext);
        callback(null, response);
      },
    });

    // Give Client ._stream() methods a way to export their context data
    // (obj.context) like they can with ._query().
  ...
  pipe(writable, options) {
    if (!writable.writableObjectMode) {
      throw new Error('Destination stream must be objectMode: true');
    }
  ...
  async query(obj) {
    ...
    this.builder.emit('query', Object.assign({ __knexUid, __knexTxId }, query));
    ...
    let queryPromise = this.client.query(this.connection, query);

    if (query.timeout) {
      queryPromise = timeout(queryPromise, query.timeout);
    }

    const resultMapper = this.client.resultMapper(this.builder);

    // Await the return value of queryPRomise; in the case of sqlite3's
    ...
    return queryPromise
      .then(async (query) => {
        ...
        if (query.output) {
          ...
          processedResponse = this.client.processResponse(query);
        }

        if (resultMapper !== undefined) {
          if (typeof obj.pluck === 'string') {
            // processResponse converts consistent data (array of rows) into
            // a knex-desired shape. it'd be better to apply that behavior
            // in a central place instead of every dialect, but that's a refactor
            // for another day.
            //
            // for now, we have to deal with the variance of the data returned
            // by processResponse...
            resultMapper.applyToPlucked(
              obj.pluck,
              processedResponse,
              query.context
            );
          } else {
            // cast column values on the result data
            resultMapper.applyTo(processedResponse, query.context);
          }
        }
        ...
        this.builder.emit(
          ...
          Object.assign({ __knexUid, __knexTxId }, query),
          this.builder
        );
      ...
      .catch((error) => {
        ...
        const { timeout, sql, bindings } = query;

        let cancelQuery;
        if (query.cancelOnTimeout) {
      ...
      .catch((error) => {
        this.builder.emit(
          'query-error',
          error,
          Object.assign({ __knexUid, __knexTxId, queryContext }, query)
        );
        throw error;
      });
  }
