...
class Runner {
  ...
  async query(obj) {
    ...
    const taggedObj = tracker.withIds(connection, obj);
    this.builder.emit('query', taggedObj);
    ...
    let queryPromise = this.client.query(connection, obj);
    ...
    return queryPromise
      ...
      .catch((error) => {
        ...
        if (obj.cancelOnTimeout) {
          cancelQuery = this.client.cancelQuery(connection);
        } else {
      ...
      .catch((error) => {
        const taggedObj = tracker.withIds(connection, obj);
        this.builder.emit('query-error', error, taggedObj);
        throw error;
      });
  }
