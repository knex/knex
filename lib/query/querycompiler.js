...
const { supportsMultiTableDelete, getMultiTableDeleteDescription } = require('../dialects/database-capabilities');
...
class QueryCompiler {
  ...
  del() {
    ...
    // Check for multi-table delete support
    if (deleteTables && deleteTables.length > 0) {
      // Use dynamic strategy for multi-table delete
      const strategy = this._getMultiTableDeleteStrategy();
      if (strategy) {
        // Validate that all delete tables are referenced in the query
        this._validateDeleteTables(deleteTables);
        
        // Use the strategy to compile multi-table delete
        return strategy.call(this, deleteTables, withSQL, tableName, joins, wheres);
      } else {
        // Fallback: check if database has any multi-table capability
        if (this._hasMultiTableDeleteCapability()) {
          throw new Error(
            `Multi-table DELETE operations are not yet implemented for ${this.client.driverName}. ` +
            'Please implement _getMultiTableDeleteStrategy() in the dialect compiler. ' +
            `${getMultiTableDeleteDescription(this.client.driverName)}`
          );
        } else {
          throw new Error(
            `Multi-table DELETE operations are not supported by ${this.client.driverName}. ` +
            `${getMultiTableDeleteDescription(this.client.driverName)}`
          );
        }
      }
  ...
  // Get the multi-table delete strategy for this database
  // Override in dialect-specific compilers to provide implementation
  _getMultiTableDeleteStrategy() {
    return null; // Default: no strategy
  }
  
  // Check if the database has any potential for multi-table delete operations
  // Override in dialect-specific compilers
  _hasMultiTableDeleteCapability() {
    // Use the centralized capabilities system
    return supportsMultiTableDelete(this.client.driverName);
  }
  
  // Check if the current dialect supports multi-table DELETE (deprecated - use strategy pattern)
  _supportsMultiTableDelete() {
    return this._getMultiTableDeleteStrategy() !== null;
  }
  
  // Validate that delete tables are referenced in the query
  _validateDeleteTables(deleteTables) {
    // Basic validation - ensure tables are valid strings
    for (const table of deleteTables) {
      if (typeof table !== 'string' || !table.trim()) {
        throw new Error('Delete table names must be non-empty strings');
      }
    }
  }
  
  // Compile multi-table delete SQL (override in dialect-specific compilers)
  _compileMultiTableDelete(deleteTables, withSQL, tableName, joins, wheres) {
    throw new Error(
      `Multi-table DELETE compilation not implemented for ${this.client.driverName}`
    );
  }
