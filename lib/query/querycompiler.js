...
/**
 * @typedef {'select'|'first'|'insert'|'update'|'upsert'|'del'|'counter'|'truncate'|'pluck'|'columnInfo'|'raw'} QueryBuilderMethod
 * @typedef {'alterTable'|'alterView'|'add'|'alter'} SchemaBuilderMethod
 * @typedef {QueryBuilderMethod|SchemaBuilderMethod} QueryMethod
 * @typedef {import('./cast.js').CastSpec} CastSpec
 * @typedef {import('./cast.js').ResultMapper} ResultMapper
 */

/**
 * @typedef {Object} CompiledQuery The result of QueryCompiler.toSQL()
 * @property {QueryMethod} method The "kind" of query we're executing, in knex terms
 * @property {any[]} options Arbitrary data from knex.options(), passed to wrapIdentifier / postProcessResponse user hooks
 * @property {number|false} timeout False, or the duration (in ms) to wait before rejecting the query promise
 * @property {boolean} cancelOnTimeout True when the database supports cancelation
 * @property {string} sql Query to execute
 * @property {any[]} [bindings] Bindings for the query
 * @property {string} [as] Alias for a subquery
 * @property {string} [pluck] Column to pluck from query results
 * @property {{sql: string, bindings: any[]}} toNative
 * @property {string} __knexQueryUid
 */

/**
 * @template {object} Res
 * @template {object} Ctx
 * @typedef {Object} ProcessResponseQueryObject The subset of the Query object that processResponse methods consume
 * @property {any} [queryContext] User-supplied arbitrary data
 * @property {Res} response Dialect ._query() return type, consumed by dialect .processResponse()
 * @property {Ctx} [context] Dialect ._query() out-of-band data, consumed by dialect .processResponse()
 * @property {CompiledQuery['method']} method The "kind" of query we're executing, in knex terms
 * @property {string|string[]} [returning] Returning columns
 * @property {(row: any) => any} [pluck] Pluck function
 */
...
class QueryCompiler {
   ...
   * @param {import('../query/querybuilder.js')} builder
   * @param {any[]} bindings
   */
  ...
  toSQL(method, tz) {
    ...
    if (isString(val)) {
      ...
      // probably only ever assigns sql and bindings?
      assign(query, val);
    }
