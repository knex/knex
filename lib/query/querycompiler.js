  ...
  analytic(stmt) {
    ...
    if (stmt.raw) {
      ...
      if (stmt.partitions.length) {
        ...
        sql +=
          map(stmt.partitions, function (partition) {
            if (isString(partition)) return self.formatter.columnize(partition);
            else
              return (
                self.formatter.columnize(partition.column) +
      ...
      sql += map(stmt.order, function (order) {
        if (isString(order)) return self.formatter.columnize(order);
        else
          return (
            self.formatter.columnize(order.column) +
