...
class QueryCompiler {
  ...
  // Returns the WHERE SQL for an upsert (onConflict().merge()) clause,
  // excluding any WHERE clauses that existed before merge() was called.
  // This prevents pre-existing WHERE clauses from leaking into the upsert
  // when a query builder instance is reused. Fixes #5257.
  _mergeWheres() {
    const mergeWhereOffset = this.single.mergeWhereOffset || 0;
    const allWheres = this.grouped.where;
    if (!allWheres || allWheres.length <= mergeWhereOffset) {
      return '';
    }
    const savedWheres = this.grouped.where;
    this.grouped.where = allWheres.slice(mergeWhereOffset);
    const wheres = this.where();
    this.grouped.where = savedWheres;
    return wheres ? ` ${wheres}` : '';
  }
