...
class QueryCompiler {
  ...
  group() {
    ...
    for (const item of items) {
      if (item.type === 'groupByRaw') {
        sql.push(this.formatter.columnize(item.value));
        continue;
      }

      let columnOrExpression = this.formatter.columnize(item.value);
      if (item.value instanceof QueryBuilder) {
        columnOrExpression = `(${columnOrExpression})`;
      }

      sql.push(columnOrExpression);
    }
