...
class QueryCompiler {
  ...
  order() {
    ...
    const sql = items.map((item) => {
      ...
      if (item.type === 'orderByRaw') {
        return this.formatter.wrapForExpressionList(item.value);
      } else if (nulls) {
        return this._orderWithNulls(item);
      } else {
        return this._orderWithoutNulls(item);
      }
    });
  ...
  _orderWithoutNulls(item) {
    const columnOrExpression = this.formatter.wrapForExpressionList(item.value);
    ...
    return `${columnOrExpression} ${direction}`;
  }

  _orderWithNulls(item) {
    // because the formatter and direction may both push to the bindings array,
    // we have to make these thunks. clients may insert these values multiple
    // times or in varying orders, so this allows us to guarantee the bindings
    // retain the order they were "used" in, so long as clients use them in
    // the appropriate order. This is easiest accomplished by using them directly
    // in string construction, e.g.:
    //
    // return `${column()} ${direction()}`
    const columnOrExpressionThunk = () =>
      this.formatter.wrapForExpressionList(item.value);

    const directionThunk = () =>
      direction_(
        item.direction,
        this.builder,
        this.client,
    ...
    return this._orderByHandler(
      columnOrExpressionThunk,
      directionThunk,
      item.nulls
    );
  }
   ...
   *
   * @abstract
   * @param {import('./orderby-clause').Expr} columnOrExpression
   * @param {import('./orderby-clause').Dir} direction
   * @param {import('./orderby-clause').NullOrder} nulls
   */
  _orderByHandler(columnOrExpression, direction, nulls) {
    throw new Error(
