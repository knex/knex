...
class QueryCompiler {
  ...
  _basicGroupOrder(item, type) {
    ...
    const direction =
      type === 'order' && item.type !== 'orderByRaw'
        ...
        : '';
    if (item.nulls && direction) {
      return column + ', ' + this.formatter.columnize(item.value) + direction;
    }
  ...
  _groupOrderNulls(item, type) {
    ...
    if (item.nulls && !(item.value instanceof Raw)) {
      return `${column}${direction ? direction : ''} nulls ${
        item.nulls
      }, ${this.formatter.columnize(item.value)}${direction}`;
    }
    return column + direction;
  }
