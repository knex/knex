...
class QueryCompiler {
  ...
  _preValidate() {
    ...
    const verb = HOP.call(methodAliases, method)
      ...
      : method;
    if (!HOP.call(invalidClauses, verb)) return;
    ...
    for (let i = 0; i < invalid.length; i++) {
      ...
      const hasNonEmptyGrouped =
        HOP.call(this.grouped, clause) && this.grouped[clause].length > 0;
      const hasNonEmptySingle =
        HOP.call(this.single, clause) && this.single[clause] != null;

      if (hasNonEmptyGrouped || hasNonEmptySingle) {
        throw new Error(
          `Refused to compile query: \`${clause}\` has no effect on a \`${verb}\` statement`
        );
      }
    }
  }
