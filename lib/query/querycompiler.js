...
class QueryCompiler {
  ...
  order() {
    ...
    for (const item of items) {
      ...
      if (item.nulls && this._orderByHandler !== undefined) {
        const handler = this._orderByHandler;
        switch (handler.type) {
          case 'emulated':
            sql.push(handler.fn.call(this, columnOrExpression, item.nulls));
            break;
          case 'native':
            direction = handler.fn.call(this, direction, item.nulls);
            break;
        }
      }
