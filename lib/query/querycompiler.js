...
const { AlwaysFail } = require('./orderby-clause');
...
class QueryCompiler {
  ...
  group() {
    const items = this.grouped['group'];
    if (!items) return '';

    // each item represents a call to .groupBy() on a query builder
    // each item _contains_ one or more columns/expressions given to
    // qb.groupBy(). unpack them all into a single array and format
    // them for the query.
    const sql = items
      .flatMap((item) => item.value)
      .map((item) => this.formatter.wrapForExpressionList(item));

    return sql.length ? 'group by ' + sql.join(', ') : '';
  }

  // Compiles the `order by` clause
  order() {
    const items = this.grouped['order'];
    if (!items) return '';

    // each item represents a call to .orderBy() on a query builder
    // each item _contains_ exactly one column/expression given to
    // qb.orderBy(). accumulate an array of formatted SQL for each
    // "order by" specification
    const sql = [];
    for (const item of items) {
      // orderByRaw ignores "order by nulls" handling
      if (item.type === 'orderByRaw') {
        sql.push(this.formatter.wrapForExpressionList(item.value));
        continue;
      }

      // we get all sorts of things here from literal values to query builders
      // query builders and callbacks are interpreted as subqueries, but in order
      // to emulate "order by nulls" we may have to specify them more than once.
      // capture the "thing we are ordering by" so that it can be repeated by
      // order by handlers if necessary
      const columnOrExpression = this.formatter.wrapForExpressionList(
        item.value
      );

      // the direction to order the actual column / subquery result by.
      // might be altered by the order-by handler (e.g. to turn "foo asc"
      // into "foo asc nulls first")
      ...
      // the placement of null values
      const nulls = item?.nulls ?? '';

      if (nulls !== 'first' && nulls !== 'last' && nulls !== '') {
        throw new Error(`Unknown 'nulls' value in order by: ${item?.nulls}`);
      }

      // apply the order-by handler if any null-ordering behavior was specified
      if (nulls) {
        const handler = this._orderByHandler;
        switch (handler.type) {
          case 'emulated':
            // for dialects that don't support "nulls first" as part of the order
            // specification, we push an extra item onto the order-by list that
            // explicitly sorts the result based on the nullness of the value.
            // this comes before the actual column order which gets pushed at
            // the end of the loop below.
            sql.push(handler.fn.call(this, columnOrExpression, nulls));
            break;
          case 'native':
            // for dialects that _do_ support nulls first, give them a chance to
            // specify how they express that syntax
            direction = handler.fn.call(this, direction, nulls);
            break;
        }
      }

      sql.push(`${columnOrExpression} ${direction}`);
    }
  ...
  /**
   * @abstract
   * @type {import('./orderby-clause').OrderByHandler}
   */
  _orderByHandler = AlwaysFail;
