...
const HOP = require('../util/security').hasOwnProperty;
...
class QueryCompiler {
  ...
  _preValidate() {
    ...
    const verb = HOP.call(methodAliases, method)
      ? methodAliases[method]
      : method;
    if (!HOP.call(invalidGroups, verb)) return;
    ...
    const invalid = invalidGroups[verb];
    for (let i = 0; i < invalid.length; i++) {
      const group = invalid[i];
      if (!HOP.call(this.grouped, group) || this.grouped[group].length === 0) {
        continue;
      }

      throw new Error(
        `Refused to compile query: \`${group}\` has no effect on a \`${verb}\` statement`
      );
    }
  }
