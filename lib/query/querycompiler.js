...
class QueryCompiler {
  ...
  select() {
    ...
    const wrap = this._needWrapMainQuery() && !onlyUnions;
    const allStatements =
      (wrap ? '(' : '') + compact(statements).join(' ') + (wrap ? ')' : '');
  ...
  union() {
    ...
    if (!unions) return '';
    let sql = '';
    const mainQueryWrapped = arguments[0]
      ? arguments[0]._needWrapMainQuery()
      : false;
    for (let i = 0, l = unions.length; i < l; i++) {
      ...
      if (statement) {
        const wrap = mainQueryWrapped || union.wrap;
        if (wrap) sql += '(';
        sql += statement;
        if (wrap) sql += ')';
      }
