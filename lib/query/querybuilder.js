...
class Builder extends EventEmitter {
  ...
  delete(ret, options) {
    ...
    // Handle new multi-table delete syntax: del(tables, returning, options)
    if (Array.isArray(ret) || (typeof ret === 'string' && arguments.length > 1)) {
      // ret is actually tables parameter
      this._single.deleteTables = Array.isArray(ret) ? ret : [ret];
      const returning = arguments[1];
      const opts = arguments[2];
      if (!isEmpty(returning)) this.returning(returning, opts);
    } else {
      // Original single-table delete syntax: del(returning, options)
      if (!isEmpty(ret)) this.returning(ret, options);
    }
