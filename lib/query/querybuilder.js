...
class OnConflictBuilder {
  ...
  merge(updates) {
    ...
    // Record the current WHERE clause count. Only WHERE clauses added
    // after this point should be applied to the upsert operation.
    // This prevents pre-existing WHERE clauses from leaking into the
    // upsert statement when a builder instance is reused. Fixes #5257.
    this.builder._single.mergeWhereOffset = this.builder._statements.filter(
      (s) => s.grouping === 'where'
    ).length;
    return this.builder;
  }
