...
class Builder extends EventEmitter {
  ...
  normaliseNumber(value) {
    ...
    if (isNaN(floatedValue)) {
      throw new Error(`A valid number must be provided to increment`);
    }

    if (floatedValue < Math.MAX_SAFE_INTEGER) {
      return floatedValue;
    }

    throw new Error("Value must be less than Number.MAX_SAFE_INTEGER for accurate updates");
  ...
  decrement(column, amount = 1) {
    const normalisedNumber = this.normaliseNumber(amount);
    console.log(normalisedNumber)
    if (isObject(column)) {
      for (const key in column) {
        this._counter(key, -column[key]);
      }
  ...
  _counter(column, amount) {
    ...
    const normalisedAmount = parseFloat(amount);

    this._single.counter[column] = normalisedAmount;

    console.log(this._single);
