...
class Builder extends EventEmitter {
  ...
  first() {
    if (this._method && this._method !== 'select') {
      throw new Error(
        `Cannot apply "first", response processing method is already set to "${this._method}"`
      );
    }
  ...
  pluck(column) {
    if (this._method && this._method !== 'select') {
      throw new Error(
        `Cannot apply "pluck", response processing method is already set to "${this._method}"`
      );
    }
