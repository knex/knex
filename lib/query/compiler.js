...
(0, _lodash.assign)(QueryCompiler.prototype, {
  ...
  _aggregate: function _aggregate(stmt) {
    ...
    var _ref =
      ...
      aliasSeparator =
        _ref$aliasSeparator === undefined ? ' as ' : _ref$aliasSeparator,
  ...
  aggregateRaw: function aggregateRaw(stmt) {
    ...
    return (
      stmt.method +
      '(' +
      (distinct + this.formatter.unwrapRaw(stmt.value)) +
      ')'
    );
  },
  ...
  onBetween: function onBetween(statement) {
    return (
      ...
      (0, _lodash.map)(
        statement.value,
        (0, _lodash.bind)(this.formatter.parameter, this.formatter)
      ).join(' and ')
    );
  },
  ...
  _groupsOrders: function _groupsOrders(type) {
    ...
    var sql = items.map(function(item) {
      var column =
        item.value instanceof _raw2.default
