...
(0, _lodash.assign)(QueryCompiler.prototype, {
  ...
  onWrapped(clause) {
    ...
    const wrapJoin = new _joinclause2.default();
  ...
  _prepInsert(data) {
    ...
    while (++i < data.length) {
      ...
      const keys = (0, _keys2.default)(data[i]);
      let j = -1;
      while (++j < keys.length) {
        const key = keys[j];
  ...
  _prepUpdate(data) {
    ...
    const vals = [];
    const columns = (0, _keys2.default)(data);
    let i = -1;
    while (++i < columns.length) {
  ...
  _groupsOrders(type) {
    ...
    const sql = items.map(item => {
      const column = item.value instanceof _raw2.default ? formatter.unwrapRaw(item.value) : formatter.columnize(item.value);
