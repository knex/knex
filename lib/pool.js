...
function connectionErrorHandler(pool, connection, err) {
  if (connection && err && err.fatal) {
    if (connection.__knex__disposed) return;
    connection.__knex__disposed = true;
    pool.genericPool.destroy(connection);
  }
}
      ...
      pool.client.acquireRawConnection()
        .tap(function(connection) {
          if ('function' === typeof connection.on) {
            var errorHandler = connectionErrorHandler.bind(null, pool, connection);
            connection.on('error', errorHandler);
            connection.on('end',   errorHandler);
          }
