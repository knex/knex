...
function Knex(config) {
  if (typeof config === 'string') {
    return new Knex(
      (0, _lodash.assign)((0, _parseConnection2.default)(config), arguments[2])
    );
  }
  var Dialect = void 0;
  if (arguments.length === 0 || (!config.client && !config.dialect)) {
    ...
    config.client.prototype instanceof _client2.default
  ) {
    Dialect = config.client;
  } else {
    var clientName = config.client || config.dialect;
    Dialect = require('./dialects/' +
      (aliases[clientName] || clientName) +
      '/index.js');
  }
