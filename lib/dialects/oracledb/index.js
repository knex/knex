...
class Client_Oracledb extends Client_Oracle {
  constructor(config) {
    ...
    if (this.version) {
      ...
      this.version = this._parseVersion(this.version);
    }
  ...
  acquireRawConnection() {
    ...
    return new Promise(function (resolver, rejecter) {
      ...
      client.driver.getConnection(oracleDbConfig, function (err, connection) {
        ...
        if (!client.version) {
          resolver(
            client.checkVersion(connection).then(function (version) {
  ...
  async checkVersion(connection) {
    ...
    return new Promise((resolver, rejecter) => {
      connection.execute(`select BANNER from V$VERSION`, (err, response) => {
        if (err) {
          return rejecter(err);
        }

        const versionString = response.rows[0][0];
        const detectedVersion = this._parseVersion(versionString);

        if (!detectedVersion) {
          return rejecter(
            // When original version is set to null, user-provided version was invalid and we fell-back to auto-detect
            // Otherwise, we couldn't auto-detect.
            new Error(
              this.version === null
                ? 'Invalid Oracledb version number format passed to knex. Unable to successfully auto-detect as fallback.'
                : 'Unable to detect Oracledb version number automatically. Please specify the version in knex configuration.'
            )
          );
        }

        resolver(detectedVersion);
      });
    });
  }
  ...
  _parseVersion(versionString) {
    try {
      const version = /^\d/.test(versionString)
        ? versionString
        : /^Oracle Database .* ((?:\d+\.){4}\d+).*$/.exec(versionString)[1];
      ...
      // Regex match returned null when expected a match group: bad version format.
      return null;
    }
  }
