...
class Client_PG extends Client {
  ...
  positionBindings(sql) {
    ...
    return sql.replace(/(\\*)(\?)/g, function (match, escapes) {
      // If we detect a "?".. we need to convert that into a string literal.
  ...
  _query(connection, obj) {
    ...
    // DRAFT
    const needsPgFormat = queryConfig.text.includes('"$') || queryConfig.text.includes('\'$');

    const query = needsPgFormat ? pgformat(queryConfig.text, queryConfig.values) : queryConfig;

    console.log(query, needsPgFormat, 'query');
