...
class Client_PG extends Client {
  ...
  _parseVersion(versionString) {
    const match = /^PostgreSQL (.*?)( |$)/.exec(versionString);
    if (match) return match[1];
    // PostgreSQL-compatible engines (e.g. GaussDB, openGauss, AlloyDB,
    // CockroachDB) return their own banner from `SELECT version()`.
    // Fall back to a conservative PG version so feature detection still
    // works against those engines instead of throwing.
    return '9.2.4';
  }
