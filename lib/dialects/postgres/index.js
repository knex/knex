...
(0, _lodash.assign)(Client_PG.prototype, {
  ...
  setSchemaSearchPath: function setSchemaSearchPath(connection, searchPath) {
    ...
    if (!(0, _lodash.isArray)(path) && !(0, _lodash.isString)(path)) {
      throw new TypeError(
        'knex: Expected searchPath to be Array/String, got: ' +
          (typeof path === 'undefined'
            ? 'undefined'
            : (0, _typeof3.default)(path))
      );
    }
    ...
    if ((0, _lodash.isString)(path)) {
      if ((0, _lodash.includes)(path, ',')) {
        ...
        var arraySyntax =
          '[' +
          (0, _lodash.map)(parts, function(searchPath) {
            return "'" + searchPath + "'";
          }).join(', ') +
          ']';
        this.logger.warn(
          'Detected comma in searchPath "' +
            path +
            '".' +
            ('If you are trying to specify multiple schemas, use Array syntax: ' +
              arraySyntax)
        );
      }
      path = [path];
  ...
  _stream: function _stream(connection, obj, stream, options) {
    var PGQueryStream = process.browser
      ? undefined
      : require('pg-query-stream');
    var sql = obj.sql;

    return new _bluebird2.default(function(resolver, rejecter) {
      var queryStream = connection.query(
...
function arrayString(arr, esc) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    if (val === null || typeof val === 'undefined') {
      ...
      result += (0, _stringify2.default)(
        typeof val === 'string' ? val : esc(val)
      );
    }
  }
  return result + '}';
