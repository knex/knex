...
class Client_PG extends Client {
  ...
  positionBindings(sql) {
    let questionCount = 0;
    // TODO: Look to consolidate REGEX in different dialects
    return sql.replace(/(\\*)(\?)/g, function (match, escapes) {
      console.log(match, escapes, 'match and escapes');
      if (escapes.length % 2) {
        return '?';
      } else {
        questionCount++;
  ...
  _query(connection, obj) {
    ...
    // Testing out use of pgformat
    const needsPgFormat = /(['"])\$[0-9]+\1/.test(obj.sql);

    console.log(needsPgFormat, 'needsPgFormat');

    // select * from "users" where json_key.json_value @? '$.*.bar ? (@ == "$1")'
    const realQConfig = needsPgFormat ? { text: pgformat(obj.sql, obj.bindings) } : queryConfig;

    console.log(realQConfig, 'realQConfig');

    return new Promise(function (resolver, rejecter) {
      connection.query(realQConfig, function (err, response) {
        if (err) return rejecter(err);
