...
class QueryCompiler_PG extends QueryCompiler {
  ...
  insert() {
    ...
    const { returning, ignore, merge, insert } = this.single;
    if (returning) sql += this._returning(returning);
    if (ignore) sql += this._ignore(ignore);
    if (merge) sql += this._merge(merge.updates, merge.columns, insert);
  ...
  _returning(value) {
    return ` returning ${this.formatter.columnize(value)}`;
  }
