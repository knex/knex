...
assign(QueryCompiler_PG.prototype, {
  ...
  columnInfo: function columnInfo() {
    ...
    var sql = 'select c.column_name, c.udt_schema, c.character_maximum_length, ' + 'c.is_nullable, c.column_default, ' + 'format_type(t.oid, null) as type ' + 'from information_schema.columns c ' + 'join pg_catalog.pg_type t on c.udt_name = t.typname ' + 'where c.table_name = ? and c.table_catalog = ?';
      ...
      output: function output(resp) {
        var out = _.reduce(resp.rows, function (columns, val) {
          columns[val.column_name] = {
            type: (val.udt_schema === 'pg_catalog' ? '' : val.udt_schema + '.') + val.type,
