...
class QueryCompiler_PG extends QueryCompiler {
  ...
  _whereJsonHas(statement, operatorHas) {
    const arrayValues = Array.isArray(statement.values)
      ? statement.values
          .map(function (value) {
          ...
          .join(',')
      : [statement.values];
    const operator = arrayValues.length === 1 ? '?' : operatorHas;
    return this._not(
      statement,
      `${columnize_(
        statement.column,
