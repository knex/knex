...
class TableCompiler_PG extends TableCompiler {
  ...
  index(columns, indexName, options) {
    ...
    // Build column list, supporting per-column operator classes:
    // columns can be a mix of strings and { column, operator } objects.
    // e.g. ['name', { column: 'bio', operator: 'gin_trgm_ops' }]
    const columnList = (Array.isArray(columns) ? columns : [columns])
      .map((col) => {
        if (isObject(col) && col.column) {
          const wrapped = this.formatter.wrap(col.column);
          return col.operator ? `${wrapped} ${col.operator}` : wrapped;
        }
        return this.formatter.wrap(col);
      })
      .join(', ');
