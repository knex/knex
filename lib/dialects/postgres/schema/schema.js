...
_.each([
  'searchPath', 'createSchema', 'dropSchema', 'hasSchema'
], function(method) {
...
SchemaCompiler_PG.prototype.hasSchema = function(schemaName) {
  this.pushQuery({
    sql: 'select * from pg_catalog.pg_namespace where nspname = ?',
    bindings: [schemaName],
    output: function(resp) {
      return resp.rows.length > 0;
    }
