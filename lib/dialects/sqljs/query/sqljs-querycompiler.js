...
class QueryCompiler_SQLJS extends QueryCompiler {
  constructor(client, builder, formatter) {
    super(client, builder, formatter);

    // The locks are not applicable in SQLJS
  ...
  // SQLJS requires us to build the multi-row insert as a listing of select with
      ...
      insertData.values.forEach((bindings) => {
        each(bindings, (binding) => {
          if (binding === undefined)
            throw new TypeError(
              '`sqljs` does not support inserting default values. Specify ' +
  ...
  whereJsonSupersetOf(statement) {
    throw new Error(
      'Json superset where clause not actually supported by SQLJS'
    );
  }

  whereJsonSubsetOf(statement) {
    throw new Error('Json subset where clause not actually supported by SQLJS');
  }
...
module.exports = QueryCompiler_SQLJS;
