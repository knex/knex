...
class TableCompiler_SQLJS extends TableCompiler {
  constructor() {
    super(...arguments);
  }
  ...
  unique(columns, indexName) {
    ...
    if (deferrable && deferrable !== 'not deferrable') {
      this.client.logger.warn(
        `sqljs: unique index \`${indexName}\` will not be deferrable ${deferrable} because sqljs does not support deferred constraints.`
      );
    }
    indexName = indexName
      ? this.formatter.wrap(indexName)
  ...
  primary(columns, constraintName) {
    ...
    if (deferrable && deferrable !== 'not deferrable') {
      this.client.logger.warn(
        `sqljs: primary key constraint \`${constraintName}\` will not be deferrable ${deferrable} because sqljs does not support deferred constraints.`
      );
    }
    constraintName = this.client.customWrapIdentifier(constraintName, identity);
    ...
    if (pks.length > 0 && pks[0].args.length > 0) {
      ...
      const needUniqueCols =
        this.grouped.columns.filter((t) => t.builder._type === 'increments')
          .length > 0;
      // SQLJS dont support autoincrement columns and composite primary keys (autoincrement is always primary key).
...
module.exports = TableCompiler_SQLJS;
