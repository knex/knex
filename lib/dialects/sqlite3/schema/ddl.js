...
class SQLite3_DDL {
  ...
  async generateReinsertCommands(createTable, newSql, mapRow) {
    ...
    /*
    TODO Replace with the bulk copy from table to table

    const copyDataCommands = await copyData(this.trx, this.tableName(), this.tableName())
    if (copyDataCommands) {
      result.push(...copyDataCommands)
    }
     */
    ...
    const copyDataCommands = await copyData(
      this.trx,
      this.tableName(),
      this.tableName()
    );
    ...
    const reinsertDataCommands = await reinsertData(this.trx, mapRow, this.tableName(), this.alteredName)
    if (reinsertDataCommands) {
      result.push(...reinsertDataCommands);
    }
     */
