  ...
  primary: async function (columns, constraintName) {
    return this.client.transaction(
      async (trx) => {
        ...
        let newColumnDefinitions = columnDefinitions
          .split(/,(?![^(]*\))/g) // comma, except when in parentheses
          .map((line) => line.trim())
          .filter((line) => line.startsWith('primary') === false)
          .map((line) => line.replace(/primary key/i, ''));

        let primaryKeyDef = `primary key(${columns.join(',')})`;

        if (constraintName) {
          primaryKeyDef = `constraint ${constraintName} ${primaryKeyDef}`;
        }

        newColumnDefinitions.push(primaryKeyDef);
        newColumnDefinitions = newColumnDefinitions.join(', ');
