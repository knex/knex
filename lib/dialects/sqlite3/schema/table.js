...
TableCompiler_SQLite3.prototype.unique = function(columns, indexName) {
  ...
  return 'create unique index ' + indexName + ' on ' + this.tableName() + ' (' + columns + ')';
};
...
TableCompiler_SQLite3.prototype.index = function(columns, indexName) {
  ...
  columns = this.formatter.columnize(columns);
  return 'create index ' + indexName + ' on ' + this.tableName() + ' (' + columns + ')';
};
...
TableCompiler_SQLite3.prototype.renameColumn = function(from, to) {
  ...
  this.pushQuery({
    ...
    output: function(pragma) {
      return new SQLite3_DDL(this, tableCompiler, pragma).renameColumn(from, to);
    }
  });
};
...
TableCompiler_SQLite3.prototype.dropColumn = function(column) {
  ...
  this.pushQuery({
    sql: 'PRAGMA table_info(' + this.tableName + ')',
    output: function(pragma) {
      return new SQLite3_DDL(this, tableCompiler, pragma).dropColumn(column);
    }
  });
};
