...
class QueryCompiler_SQLite3 extends QueryCompiler {
  ...
  insert() {
    ...
    if (insertData.values.length === 1) {
      ...
      const { ignore, merge } = this.single;
      if (ignore) sql += this._ignore(ignore);
      else if (merge)
        sql += this._merge(merge.updates, merge.columns, insertValues);
    ...
    const { ignore, merge } = this.single;
    if (ignore) sql += ' where true' + this._ignore(ignore);
    else if (merge)
      sql +=
        ' where true' + this._merge(merge.updates, merge.columns, insertValues);
