...
(0, _lodash.assign)(QueryCompiler_Oracle.prototype, {
  ...
  insert: function insert() {
    ...
    if (
      ...
      (0, _lodash.isPlainObject)(this.single.insert)
    ) {
    ...
    if (
      ...
      (0, _lodash.isEmpty)(insertValues[0])
    ) {
    ...
    if (
      (0, _lodash.isEmpty)(this.single.insert) &&
    ...
    sql.sql =
      'begin ' +
      (0, _lodash.map)(insertData.values, function(value) {
        var returningHelper = void 0;
        var parameterizedValues = !insertDefaultsOnly
          ? _this.formatter.parameterize(value, _this.client.valueForUndefined)
          : '';
        var returningValues = Array.isArray(returning)
          ? returning
          : [returning];
        var subSql = 'insert into ' + _this.tableName + ' ';

        if (returning) {
          returningHelper = new _utils.ReturningHelper(
        ...
        if (insertDefaultsOnly) {
          ...
          subSql +=
            '(' +
            _this.formatter.wrap(_this.single.returning) +
            ') values (default)';
        } else {
          subSql +=
            '(' +
            _this.formatter.columnize(insertData.columns) +
            ') values (' +
            parameterizedValues +
            ')';
        }
        subSql += returning
          ? ' returning ROWID into ' +
            _this.formatter.parameter(returningHelper)
          : '';
        ...
        var parameterizedValuesWithoutDefault = parameterizedValues
          .replace('DEFAULT, ', '')
          .replace(', DEFAULT', '');
        return (
          "execute immediate '" +
  ...
  _addReturningToSqlAndConvert: function _addReturningToSqlAndConvert(
    sql,
    returning,
    tableName
  ) {
  ...
  _surroundQueryWithLimitAndOffset: function _surroundQueryWithLimitAndOffset(
    query
  ) {
    ...
    if (hasLimit && !offset) {
      return (
        ...
        query +
        ') where rownum <= ' +
        this.formatter.parameter(limit)
      );
    }
