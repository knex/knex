...
const trigger = {
  ...
  createAutoIncrementTrigger: function (logger, tableName, schemaName) {
    const schemaText = schemaName ? `"${schemaName}".` : '';
    const triggerName = utils.generateCombinedName(
      logger,
      'autoinc_trg',
      tableName
    ...
    return (
      ...
      `  AND cons.constraint_name = cols.constraint_name` +
      `  AND cons.owner = cols.owner` +
      `  AND cols.table_name = '${schemaText}${tableName}';` +
      `  execute immediate ('create or replace trigger "${triggerName}"` +
      `  BEFORE INSERT on "${schemaText}${tableName}"` +
      `  for each row` +
      `  declare` +
      `  checking number := 1;` +
      `  begin` +
      ...
      `        select count("' || PK_NAME || '") into checking from "${schemaText}${tableName}"` +
