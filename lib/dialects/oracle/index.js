...
class Client_Oracle extends Client {
  ...
  _stream(connection, obj, stream, options = {}) {
    ...
    return new Promise(function (resolver, rejecter) {
      ...
      try {
        // oracledb strictly validates its arguments, so we
        // can't get away with just passing "undefined" for
        // options
        const args = [obj.sql, obj.bindings];
        if (options) args.push(options);

        const queryStream = connection.queryStream.apply(connection, args);
        ...
        // catch parameter validation errors and surface them on the stream
