...
(0, _lodash.assign)(TableCompiler_MSSQL.prototype, {
  ...
  dropColumn: function dropColumn() {
    ...
    var drops = (Array.isArray(columns) ? columns : [columns]).map(function(
      column
    ) {
  ...
  renameColumn: function renameColumn(from, to) {
    this.pushQuery(
      'exec sp_rename ' +
        this.formatter.parameter(this.tableName() + '.' + from) +
        ', ' +
  ...
  unique: function unique(columns, indexName) {
    ...
    var whereAllTheColumnsAreNotNull = columns
      .map(function(column) {
