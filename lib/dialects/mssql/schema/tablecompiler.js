...
(0, _lodash.assign)(TableCompiler_MSSQL.prototype, {
  ...
  alterColumns: function alterColumns(columns, builders) {
    for (var i = 0; i < columns.sql.length; i++) {
      var column = columns.sql[i];

      this.pushQuery({
        sql: "ALTER TABLE " + this.tableName() + " " + this.alterColumnPrefix + column,
        bindings: []
      });
    }
  },
