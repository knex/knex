...
class Transaction_MSSQL extends _transaction2.default {
  ...
  savepoint(conn) {
    ...
    return _bluebird2.default.resolve().then(() => this.query(conn, `SAVE TRANSACTION ${this.txid}`));
  }
  ...
  rollback(conn, error) {
    ...
    return conn.tx_.rollback().then(() => this._rejecter(error), err => {
  ...
  rollbackTo(conn, error) {
    ...
    return _bluebird2.default.resolve().then(() => this.query(conn, `ROLLBACK TRANSACTION ${this.txid}`, 2, error)).then(() => this._rejecter(error));
  }
