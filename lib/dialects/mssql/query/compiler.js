...
class QueryCompiler_MSSQL extends QueryCompiler {
  ...
  insert() {
    ...
    let sql = this.with() + `${this._buildReturningDeclare(returning)}insert into ${this.tableName} `;
  ...
  del() {
    ...
    return {
      sql:
        withSQL +
        `${this._buildReturningDeclare(returning)}delete from ${tableName}` +
  ...
  _returning(method, value, returning) {
    switch (method) {
      case 'update':
      case 'insert':
        return value
          ? `output ${this.formatter.columnizeWithPrefix('inserted.', value)} into @out`
          : '';
      case 'del':
        return value
          ? `output ${this.formatter.columnizeWithPrefix('deleted.', value)} into @out`
          : '';
      case 'rowcount':
  ...
  _buildReturningDeclare(values) {
    if (values && values.length > 0) {
      let columns = "";

      if (Array.isArray(values)) {
        columns = values.map(value => `[${value}] nvarchar(max) null`).join(",");
      } else {
        columns = `[${values}] nvarchar(max) null`
      }

      return `declare @out table (${columns}); `;
    }
  ...
  _buildReturningSelect(values) {
    if (values && values.length > 0) {
      ...
      if (Array.isArray(values)) {
        selections = values.map(value => `[${value}]`).join(",");
      } else {
        selections = `[${values}]`
      }

      return `; select ${selections} from @out`
    }
