...
class QueryCompiler_MSSQL extends QueryCompiler {
  ...
  insert() {
    ...
    let sql = this.with() + `${returning ? `declare @out table (${returning
      .map(value => `[${value}] nvarchar(max) null`).join(",")}); ` : ''}insert into ${this.tableName} `;
    ...
    if (returning) {
      sql += `; select ${returning.map(value => `[${value}]`).join(",")} from @out`;
    }
  ...
  del() {
    ...
    return {
      sql:
        withSQL +
        `${returning ? `declare @out table (${returning.map(value => `[${value}] nvarchar(max) null`)
          .join(",")}); ` : ''}delete from ${tableName}` +
        (returning ? ` ${this._returning('del', returning)}` : '') +
        (wheres ? ` ${wheres}` : '') +
        (!returning ? this._returning('rowcount', '@@rowcount') : `; select ${returning.map(value => `[${value}]`).join(",")} from @out`),
      returning: returning || '@@rowcount',
    };
  }
