...
class QueryCompiler_MSSQL extends QueryCompiler {
  ...
  _buildReturningDeclare(values) {
    if (values) {
      const columns = returning.map(value => `[${value}] nvarchar(max) null`);
      return `declare @out table (${columns.join(",")}); `;
    }
  }

  _buildReturningSelect(values) {
    if (values) {
      const selections = returning.map(value => `[${value}]`);
      return `; select ${selections.join(",")} from @out`
    }
  }
