...
const { UseExpression } = require('../../../query/orderby-clause.js');
...
class QueryCompiler_MSSQL extends QueryCompiler {
  ...
  /**
   * @type {import('../../query/orderby-clause').OrderByHandler}
   */
  _orderByHandler = UseExpression((columnOrExpression, nulls) => {
    const order = nulls === 'first' ? 'desc' : 'asc';
    return `IIF(${columnOrExpression} is null,1,0) ${order}`;
  });
