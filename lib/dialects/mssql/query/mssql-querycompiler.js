...
class QueryCompiler_MSSQL extends QueryCompiler {
  ...
  _hasMultiTableDeleteCapability() {
    return true; // MSSQL supports multi-table DELETE
  }

  // Get the multi-table delete strategy for MSSQL
  _getMultiTableDeleteStrategy() {
    return this._compileMultiTableDelete_MSSQL;
  }

  // MSSQL-specific multi-table DELETE implementation strategy
  _compileMultiTableDelete_MSSQL(deleteTables, withSQL, tableName, joins, wheres) {
    // MSSQL syntax: DELETE table1, table2 FROM table1 JOIN table2 WHERE ...
      ...
      return {
        sql:
          ...
          `${this._buildTempTable(
            returning
          )}delete ${tableList} from ${tableName}` +
          (joins ? ` ${joins}` : '') +
          (wheres ? ` ${wheres}` : '') +
          (!returning
            ? this._returning('rowcount', '@@rowcount')
  ...
  // Compile multi-table delete for MSSQL (legacy - delegates to strategy)
  _compileMultiTableDelete(deleteTables, withSQL, tableName, joins, wheres) {
    return this._compileMultiTableDelete_MSSQL(deleteTables, withSQL, tableName, joins, wheres);
  }
