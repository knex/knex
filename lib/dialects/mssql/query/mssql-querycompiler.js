...
const { spacePrefixedThunkResult } = require('../../../query/orderby-clause');
...
class QueryCompiler_MSSQL extends QueryCompiler {
  ...
  _orderByHandler(columnOrExpression, direction, nulls) {
    const nullOrder = nulls === 'first' ? 'desc' : 'asc';

    return `IIF(${columnOrExpression()} is null,1,0) ${nullOrder}, ${columnOrExpression()}${spacePrefixedThunkResult(
      direction
    )}`;
  }
