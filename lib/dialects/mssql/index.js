  ...
  _setReqInput(req, i, binding) {
    if (typeof binding == 'number') {
      if (binding % 1 !== 0) {
        let precision = 38;
        let scale = 10;
        const bindingParts = binding.toString().split('.');
        if (bindingParts.length === 2) {
          scale = Math.min(bindingParts[1].length, scale);
          precision = Math.min(
            bindingParts[0].length + bindingParts[1].length,
            precision
          );
        }
        // see https://github.com/tediousjs/tedious/blob/master/src/data-types/decimal.ts
        const scaledValue = Math.round(Math.abs(binding * Math.pow(10, scale)));
        // see https://github.com/tediousjs/tedious/issues/1058
        if (scaledValue > MAX_UINT64) {
          throw new Error(
            `p${i} cannot be stored by mssql driver, see https://github.com/tediousjs/tedious/issues/1058`
          );
        }
        req.input(`p${i}`, this.driver.Decimal(precision, scale), binding);
