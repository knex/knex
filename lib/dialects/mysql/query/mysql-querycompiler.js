...
class QueryCompiler_MySQL extends QueryCompiler {
  ...
  _hasMultiTableDeleteCapability() {
    return true; // MySQL supports multi-table DELETE
  }

  // Get the multi-table delete strategy for MySQL
  _getMultiTableDeleteStrategy() {
    return this._compileMultiTableDelete_MySQL;
  }

  // MySQL-specific multi-table DELETE implementation strategy
  _compileMultiTableDelete_MySQL(deleteTables, withSQL, tableName, joins, wheres) {
    // MySQL syntax: DELETE table1, table2 FROM table1 JOIN table2 WHERE ...
    const tableList = deleteTables.map(table => this.formatter.wrap(table)).join(', ');
    const comments = this.comments();
    const commentsPrefix = comments === '' ? '' : comments + ' ';
    
    return (
      withSQL +
      commentsPrefix +
      `delete ${tableList} from ${
  ...
  // Compile multi-table delete for MySQL (legacy - delegates to strategy)
  _compileMultiTableDelete(deleteTables, withSQL, tableName, joins, wheres) {
    return this._compileMultiTableDelete_MySQL(deleteTables, withSQL, tableName, joins, wheres);
  }
