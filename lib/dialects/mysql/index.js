...
class Client_MySQL extends Client {
  ...
  castReal(value, alias) {
    return new Cast(this).cast(value, 'real', alias);
  }

  castInt(value, alias) {
    return new Cast(this).cast(value, 'unsigned', alias);
  }
  ...
  castBinary(value, alias) {
    return new Cast(this).cast(value, 'binary', alias);
  }

  castText(value, alias) {
    // MySQL's CAST has no TEXT type; CHAR is the equivalent.
