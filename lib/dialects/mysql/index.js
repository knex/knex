...
class Client_MySQL extends Client {
  ...
  processResponse(obj, runner) {
    ...
    const responseType =
    runner.builder._responseType || runner.client.config.responseType;

  if (responseType === 'extended') {
    method = ['insert', 'del', 'update', 'counter'].includes(method)
      ? 'info'
      : method;
  }
    ...
    switch (method) {
      ...
      case 'info':
        if (rows.info) {
          const match = rows.info.match(/matched:\s*(\d+)/i);
          rows.matchedRows = match ? Number(match[1]) : undefined;
        }
        return rows;
      case 'counter':
