...
const isExpressionDefault = require('./internal/isExpressionDefault');
const parseExtra = require('./internal/parseExtra');
        ...
        return compiler.getFKRefs(runner).then(([refs]) =>
          new Promise((resolve, reject) => {
            ...
            .then(function () {
              ...
              // Handle default value — expression defaults like CURRENT_TIMESTAMP
              // must not be quoted, or MySQL will reject them as invalid.
              // Fixes #1888.
              if (column.Default !== void 0 && column.Default !== null) {
                if (isExpressionDefault(column.Default, column.Extra)) {
              ...
              // Parse the Extra field for auto_increment and on update clauses
              const extra = parseExtra(column.Extra);

              // Add back the auto increment if the column has it, fix issue #2767
              if (extra.isAutoIncrement) {
                sql += ` AUTO_INCREMENT`;
              }

              // Preserve the ON UPDATE clause (e.g. ON UPDATE CURRENT_TIMESTAMP)
              // which was previously lost during rename operations
              if (extra.onUpdateValue) {
                sql += ` ON UPDATE ${extra.onUpdateValue}`;
              }
