...
var _charsetHandler = require('./charset-handler');
...
(0, _lodash.assign)(TableCompiler_MySQL.prototype, {
  createQuery: function createQuery(columns, ifNot) {
    ...
    var _getCharsetAndCollati = (0, _charsetHandler.getCharsetAndCollation)(
        this.single,
        conn,
        this.driverName
      ),
      charset = _getCharsetAndCollati.charset,
      collation = _getCharsetAndCollati.collation;
  ...
  comment: function comment(_comment) {
    this.pushQuery(
      'alter table ' + this.tableName() + " comment = '" + _comment + "'"
    );
  },
  ...
  dropIndex: function dropIndex(columns, indexName) {
    indexName = indexName
      ...
      : this._indexCommand('index', this.tableNameRaw, columns);
    this.pushQuery(
      'alter table ' + this.tableName() + ' drop index ' + indexName
    );
  },
