...
Migrate.prototype = {
  ...
  // Migrate "down" by the last "batch" of migrations that were run.
  down: function(config) {
    return this.init(config)
    .then(this.migrationData)
    .tap(validateMigrationList)
    .then(function(all, completed) {
  ...
  latestBatchNumber: function() {
    return this.knex(this.config.tableName).max('batch').then(function(obj) {
