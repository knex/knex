...
class Migrator {
  ...
  async up(config) {
    ...
    let migrationToRun;
    const name = this._getMigrationName(this.config);
    if (name) {
  ...
  _getMigrationName(config) {
    return config.name != null
      ? config.migrationSource.getMigrationName({
        file: config.name,
...
function validateMigrationList(migrationSource, migrations) {
  ...
  if (!isEmpty(diff)) {
    ...
    throw new Error(
      `The migration directory is corrupt, the following migrations are missing: ${names.join(
        ', '
      )}`
    );
  }
