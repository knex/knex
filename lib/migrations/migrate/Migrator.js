...
class Migrator {
  ...
  async latest(config) {
    ...
    if (transactionForAll) {
      return await this.knex.transaction(async (trx) => {
        await beforeAll(trx, migrations);
        const runBatchResult = await this._runBatch(migrations, 'up', trx);
        await afterAll(trx, migrations);
        return runBatchResult;
      });
    } else {
      await beforeAll(this.knex, migrations);
      const runBatchResult = await this._runBatch(migrations, 'up');
      await afterAll(this.knex, migrations);
      return runBatchResult;
    }
  }
  ...
  async up(config) {
    ...
    if (transactionForAll) {
      return await this.knex.transaction(async (trx) => {
        await beforeAll(trx, migrationsToRun);
        const runBatchResult = await this._runBatch(migrationsToRun, 'up', trx);
        await afterAll(trx, migrationsToRun);
        return runBatchResult;
      });
    } else {
      await beforeAll(this.knex, migrationsToRun);
      const runBatchResult = await this._runBatch(migrationsToRun, 'up');
      await afterAll(this.knex, migrationsToRun);
      return runBatchResult;
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      migrationListResolver
        ...
        .then(async (migrations) => {
          ...
          const afterAll = this.config.afterAll || (() => {});
          await beforeAll(this.knex, migrations);
          const runBatchResult = await this._runBatch(migrations, 'down');
          await afterAll(this.knex, migrations);
          return runBatchResult;
        })
        .then(resolve, reject);
  ...
  down(config) {
    ...
    return migrationListResolver
      ...
      .then(async ([all, completed]) => {
        ...
        const afterAll = this.config.afterAll || (() => {});
        await beforeAll(this.knex, migrationsToRun);
        const runBatchResult = await this._runBatch(migrationsToRun, 'down');
        await afterAll(this.knex, migrationsToRun);
        return runBatchResult;
      });
  }
