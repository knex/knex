...
const { isBoolean, isString, isFunction } = require('../../util/is');
const { isSQLite } = require('../../../test/util/db-helpers');

const TRANSACTION_FILTERS = {
  false_if_sqlite: (knex) => (isSQLite(knex) ? false : undefined),
};
...
class Migrator {
  ...
  // config.
  //
  // If the value is a string, expect it to be a key into the TRANSACTION_FILTERS
  // object (defined above). Decide whether to use a transaction based on the
  // result of the associated function.
  _useTransaction(migrationContent, allTransactionsDisabled) {
    ...
    if (isString(singleTransactionValue)) {
      const filterExists = Object.prototype.hasOwnProperty.call(
        TRANSACTION_FILTERS,
        singleTransactionValue
      );
      ...
      if (!filterExists) {
        throw new Error(
          `Invalid value for config.transaction: ${singleTransactionValue} (valid: ${Object.keys(
            TRANSACTION_FILTERS
          ).join(', ')})`
        );
      }

      filterResult = TRANSACTION_FILTERS[singleTransactionValue](this.knex);
    }

    return isBoolean(filterResult) ? filterResult : useTransaction;
  }
