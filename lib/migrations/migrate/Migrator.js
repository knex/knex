...
class Migrator {
  ...
  async latest(config) {
    ...
    const migrations = getDifferenceBetweenMigrations(
  ...
  async up(config) {
    ...
    const newMigrations = getDifferenceBetweenMigrations(
      this.config.migrationSource,
      completed,
      all
    );
  ...
  rollback(config, rollbackAll = false) {
    this._disableProcessing();
    return new Promise((resolve, reject) => {
      ...
      migrationListResolver
        ...
        .then((val) => {
          const [all, completed] = val;
          const completedMigrationNames = completed.map((migration) =>
            this.config.migrationSource.getMigrationName(migration)
          );
          return rollbackAll
            ? all
                .filter((migration) =>
                  completedMigrationNames.includes(
                    this.config.migrationSource.getMigrationName(migration)
                  )
                )
                .reverse()
  ...
  down(config) {
    ...
    return migrationListResolver
      ...
      .then(([all, completed]) => {
        ...
        const name = this.config.name;
        if (name) {
          migrationToRun = completedMigrations.find(
            (migration) =>
              this.config.migrationSource.getMigrationName(migration) === name
          );
          if (!migrationToRun) {
  ...
  async _runBatch(migrations, direction, trx) {
    ...
    try {
      ...
      migrations = getDifferenceBetweenMigrations(
        this.config.migrationSource,
        completed,
        migrations
      );
...
function validateMigrationList(migrationSource, migrations) {
  ...
  const diff = getDifferenceBetweenMigrations(migrationSource, completed, all);
  if (!isEmpty(diff)) {
    throw new Error(
...
// Returns the list of migrations present in `fromMigrations` but not `toMigrations`.
function getDifferenceBetweenMigrations(
  migrationSource,
  fromMigrations,
  toMigrations
) {
  return differenceWith(
    ...
    (fromMigration, toMigration) => {
      return (
        migrationSource.getMigrationName(fromMigration) ===
        migrationSource.getMigrationName(toMigration)
      );
    }
  );
}
