...
async function knexTablesExist(migrationsTableName, schemaName, trxOrKnex) {
  const lockTable = getLockTableName(migrationsTableName);
  const schemaBuilder = getSchemaBuilder(trxOrKnex, schemaName);

  const result = await Promise.all([
    schemaBuilder.hasTable(migrationsTableName),
    schemaBuilder.hasTable(lockTable),
  ]);

  return result.every((tableExists) => tableExists);
}
