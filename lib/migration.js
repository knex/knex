...
var Migration = function(Instance) {
  this.Knex   = Instance;
  _.bindAll(this, 'currentVersion', '_migrationData');
};
...
Migration.prototype = {
  ...
  // Runs a specific migration, based on the migration version
  // number.
  ...
  currentVersion: function(config) {
    return this.listCompleted(config)
      .then(function(completed) {
        var val = _.chain(completed)
          .map(function(value) { return value.split('_')[0]; })
          .max()
          .value();
...
var getMigrations = function(all, version, direction, config) {
...
var hasRun = function(versions, check) {
