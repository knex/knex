...
class Migrator {
  ...
  async latest(config) {
    ...
    const transactionForAll = await (async () => {
      const transactionsEnabled = !this.config.disableTransactions;
      if (transactionsEnabled) {
        const migrationContents = await Promise.all(
          migrations.map((migration) => {
            return this.config.migrationSource.getMigration(migration);
          })
        );
        const migrationsNotUsingTransaction = migrationContents.filter(
          (migrationContent) => {
            return !this._useTransaction(migrationContent);
          }
        );
        return migrationsNotUsingTransaction.length <= 0;
      } else {
        return false;
      }
    })();
  ...
  async rollback(config, all = false) {
    this._disableProcessing();
    try {
      this.config = getMergedConfig(
        config,
      ...
      const [
        allMigrations,
        completed,
      ] = await migrationListResolver.listAllAndCompleted(
        this.config,
        this.knex
      );
      if (!this.config.disableMigrationsListValidation) {
        ...
        ? (
            await Promise.all(
              allMigrations.map(async (migration) => {
                const migrationName = await this.config.migrationSource.getMigrationName(
                  migration
                );
            ...
            .filter(({ migration, migrationName }) => {
              return completed.includes(migrationName);
            })
            .reverse()
        : await this._getLastBatch(allMigrations, completed);
  ...
  async currentVersion(config) {
    this._disableProcessing();
    this.config = getMergedConfig(config, this.config, this.knex.client.logger);
    const completed = await migrationListResolver.listCompleted(
  ...
  async _validateMigrationStructure(migration) {
    ...
    if (
      ...
      throw new Error(
        ...
        ${JSON.stringify(migrationContent)}
        `
      );
    }
  ...
  async _waterfallBatch(batchNo, migrations, direction, trx) {
    const trxOrKnex = trx || this.knex;
    const { tableName, schemaName, disableTransactions } = this.config;

    const updateMigrationTable = async (name) => {
      trxOrKnex.disableProcessing();
      this.knex.disableProcessing();
      if (direction === 'up') {
        await trxOrKnex.into(getTableName(tableName, schemaName)).insert({
          name,
          batch: batchNo,
    ...
    const log = await migrations.reduce(
      async (previousMigrations, migration) => {
        const { getMigrationName, getMigration } = this.config.migrationSource;
        const name = getMigrationName(migration);
        const migrationContent = await getMigration(migration);
        ...
        if (
          ...
          await this._transaction(this.knex, migrationContent, direction, name);
          await updateMigrationTable(name, previousMigrations);
        } else {
          ...
          await updateMigrationTable(name);
        }
        return [...(await previousMigrations), name];
      },
      Promise.resolve([])
    );
...
function validateMigrationList(migrationSource, all, completed) {
