  ...
  async latest(config) {
    ...
    const migrationContents = await Promise.all(
      migrations.map((migration) => {
        return this.config.migrationSouce.getMigration(migration);
      })
    );
    ...
    const allMigrationsUseTransaction =
      migrationsNotUsingTransaction.length <= 0;
    const transactionForAll =
      transactionsEnabled && allMigrationsUseTransaction;
