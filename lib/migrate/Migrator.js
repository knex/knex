  ...
  async latest(config) {
    ...
    const transactionForAll = (async () => {
      ...
      if (transactionsEnabled) {
        ...
        return Boolean(migrationsNotUsingTransaction.length <= 0);
      } else {
        return false;
      }
    })();
...
<<<<<<< HEAD
    const log = await migrations.reduce(async (previousMigrations, migration) => {
      ...
      const updateMigrationTable = async () => {
        trxOrKnex.disableProcessing();
        this.knex.disableProcessing();
        const log = [...await previousMigrations, name];
        if (direction === 'up') {
          await trxOrKnex
            .into(getTableName(tableName, schemaName))
            .insert({
              name,
              batch: batchNo,
      ...
      if (!trx && this._useTransaction(migrationContent, disableTransactions)) {
        ...
        return await updateMigrationTable();
      }
