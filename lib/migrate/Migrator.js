...
class Migrator {
  ...
  async rollback(config, all = false) {
    ...
    try {
      ...
      const [
        allMigrations,
        completed,
      ] = await migrationListResolver.listAllAndCompleted(this.config, this.knex);
      if (!this.config.disableMigrationsListValidation) {
        validateMigrationList(this.config.migrationSource, [all, completed]);
      }
      const migrations = all
        ...
        : this._getLastBatch(allMigrations, completed);
  ...
  async forceFreeMigrationsLock(config) {
    ...
    const tableExists = await getSchemaBuilder(
      knex,
      schemaName
    ).hasTable(lockTableName);
    if (tableExists) {
  ...
  async _waterfallBatch(batchNo, migrations, direction, trx) {
    ...
    const updateMigrationTable = async (name) => {
      ...
      this.knex.disableProcessing();
      const log = [...await previousMigrations, name];
      if (direction === 'up') {
    ...
    const log = await migrations.reduce(async (previousMigrations, migration) => {
      ...
      if (!trx && this._useTransaction(migrationContent, disableTransactions)) {
        ...
        return await updateMigrationTable(name);
      }
