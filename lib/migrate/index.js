...
const CONFIG_DEFAULT = (0, _freeze2.default)({
  extension: 'js',
...
class Migrator {
  ...
  latest(config) {
    ...
    return this._migrationData().tap(validateMigrationList).spread((all, completed) => {
      const migrations = (0, _lodash.difference)(all, completed);

      const transactionForAll = !this.config.disableTransactions && (0, _lodash.isEmpty)((0, _lodash.filter)(migrations, name => {
        const migration = require(_path2.default.join(this._absoluteConfigDir(), name));
  ...
  rollback(config) {
    return _bluebird2.default.try(() => {
      this.config = this.setConfig(config);
      return this._migrationData().tap(validateMigrationList).then(val => this._getLastBatch(val)).then(migrations => {
        return this._runBatch((0, _lodash.map)(migrations, 'name'), 'down');
      });
    });
  }
  ...
  status(config) {
    ...
    return _bluebird2.default.all([getTable(this.knex, this.config.tableName, this.config.schemaName).select('*'), this._listAll()]).spread((db, code) => db.length - code.length);
  }
  ...
  currentVersion(config) {
    ...
    return this._listCompleted().then(completed => {
      const val = (0, _lodash.max)((0, _lodash.map)(completed, value => value.split('_')[0]));
  ...
  forceFreeMigrationsLock(config) {
    ...
    return getSchemaBuilder(this.knex, this.config.schemaName).hasTable(lockTable).then(exist => exist && this._freeLock());
  }
  ...
  _listAll(config) {
    ...
    return _bluebird2.default.promisify(_fs2.default.readdir, { context: _fs2.default })(this._absoluteConfigDir()).then(migrations => {
      return (0, _lodash.filter)(migrations, function (value) {
        const extension = _path2.default.extname(value);
  ...
  _ensureFolder() {
    ...
    return _bluebird2.default.promisify(_fs2.default.stat, { context: _fs2.default })(dir).catch(() => _bluebird2.default.promisify(_mkdirp2.default)(dir));
  }
  ...
  _ensureTable(trx = this.knex) {
    ...
    return getSchemaBuilder(trx, schemaName).hasTable(tableName).then(exists => !exists && this._createMigrationTable(tableName, schemaName, trx)).then(() => getSchemaBuilder(trx, schemaName).hasTable(lockTable)).then(exists => !exists && this._createMigrationLockTable(lockTable, trx)).then(() => getTable(trx, lockTable, this.config.schemaName).select('*')).then(data => !data.length && trx.into(lockTableWithSchema).insert({ is_locked: 0 }));
  }
  ...
  _isLocked(trx) {
    ...
    return getTable(this.knex, tableName, this.config.schemaName).transacting(trx).forUpdate().select('*').then(data => data[0].is_locked);
  }
  ...
  _runBatch(migrations, direction, trx) {
    ...
    .then(() => trx ? this._listCompleted(trx) : []).then(completed => migrations = (0, _lodash.difference)(migrations, completed)).then(() => _bluebird2.default.all((0, _lodash.map)(migrations, (0, _lodash.bind)(this._validateMigrationStructure, this)))).then(() => this._latestBatchNumber(trx)).then(batchNo => {
      if (direction === 'up') batchNo++;
      return batchNo;
    }).then(batchNo => {
      return this._waterfallBatch(batchNo, migrations, direction, trx);
    }).tap(() => this._freeLock(trx)).catch(error => {
      let cleanupReady = _bluebird2.default.resolve();

      if (error instanceof LockError) {
        // If locking error do not free the lock.
        this.knex.client.logger.warn(`Can't take lock to run migrations: ${error.message}`);
        this.knex.client.logger.warn('If you are sure migrations are not running you can release the ' + 'lock manually by deleting all the rows from migrations lock ' + 'table: ' + this._getLockTableNameWithSchema());
      } else {
  ...
  _listCompleted(trx = this.knex) {
    const { tableName, schemaName } = this.config;
    return this._ensureTable(trx).then(() => trx.from(getTableName(tableName, schemaName)).orderBy('id').select('name')).then(migrations => (0, _lodash.map)(migrations, 'name'));
  }
  ...
  _generateStubTemplate() {
    const stubPath = this.config.stub || _path2.default.join(__dirname, 'stub', this.config.extension + '.stub');
    return _bluebird2.default.promisify(_fs2.default.readFile, { context: _fs2.default })(stubPath).then(stub => (0, _lodash.template)(stub.toString(), { variable: 'd' }));
  }
  ...
  _useTransaction(migration, allTransactionsDisabled) {
    const singleTransactionValue = (0, _lodash.get)(migration, 'config.transaction');
  ...
  _waterfallBatch(batchNo, migrations, direction, trx) {
    ...
    let current = _bluebird2.default.bind({ failed: false, failedOn: 0 });
    const log = [];
  ...
  _transaction(migration, direction, name) {
    return this.knex.transaction(trx => {
      return warnPromise(migration[direction](trx, _bluebird2.default), name, () => {
        trx.commit();
      });
    });
  }
...
exports.default = Migrator; // Validates that migrations are present in the appropriate directories.
