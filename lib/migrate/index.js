...
var Migrator = (function () {
  ...
  Migrator.prototype._ensureTable = function _ensureTable() {
    ...
    return this.knex.schema.hasTable(table).then(function (exists) {
      return !exists && _this4._createMigrationTable(table);
    }).then(function () {
      return _this4.knex.schema.hasTable(lockTable);
    }).then(function (exists) {
      return !exists && _this4._createMigrationLockTable(lockTable);
    }).then(function () {
      return _this4.knex(lockTable).select('*');
    }).then(function (data) {
      return !data.length && _this4.knex(lockTable).insert({ is_locked: false });
    });
  };
  ...
  Migrator.prototype._isLocked = function _isLocked(trx) {
    ...
    return this.knex(tableName).transacting(trx).forUpdate().select('*').then(function (data) {
      ...
      if (data[0].is_locked !== false && data[0].is_locked !== '0' && data[0].is_locked !== 0) {
        return true;
      }
  ...
  Migrator.prototype._lockMigrations = function _lockMigrations(trx) {
    var tableName = this._getLockTableName(this.config.tableName);
    return this.knex(tableName).transacting(trx).update({ is_locked: true });
  };
  ...
  Migrator.prototype._runBatch = function _runBatch(migrations, direction) {
    ...
    return this.knex.transaction(function (trx) {
      return _this5._isLocked(trx).then(function (isLocked) {
        if (isLocked) {
          throw new LockError();
        }
    ...
    })['catch'](LockError, function (error) {
      ...
      return _this5._unlockMigrations()['finally'](function () {
        throw error;
      });
    });
  };
