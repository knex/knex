...
var Migrator = (function () {
  ...
  Migrator.prototype._ensureTable = function _ensureTable() {
    ...
    return this.knex.schema.hasTable(table).then(function (exists) {
      ...
      return _this4.knex.schema.hasTable(lockTable).then(function (exists) {
        if (!exists) return _this4._createMigrationLockTable(lockTable);
      });
    }).then(function () {
      return _this4.knex(lockTable).select('*').then(function (data) {
        if (!data.length) {
          return _this4.knex(lockTable).insert({ is_locked: false });
        }
      });
    });
  };
  ...
  Migrator.prototype._getLockTableName = function _getLockTableName(tableName) {
    return tableName + '_lock';
  };

  Migrator.prototype._isLocked = function _isLocked() {
    var tableName = this._getLockTableName(this.config.tableName);
    return this.knex(tableName).select('*').then(function (data) {
      if (data[0].is_locked) {
        return true;
      }
      return false;
  ...
  Migrator.prototype._lockMigrations = function _lockMigrations() {
    var tableName = this._getLockTableName(this.config.tableName);
    return this.knex(tableName).update({ is_locked: true });
  };

  Migrator.prototype._unlockMigrations = function _unlockMigrations() {
    var tableName = this._getLockTableName(this.config.tableName);
    return this.knex(tableName).update({ is_locked: false });
  };
  ...
  Migrator.prototype._runBatch = function _runBatch(migrations, direction) {
    ...
    return this._isLocked().then(function (isLocked) {
      if (isLocked) {
        throw new Error('migrations failed: migration in progress');
      }
      return Promise.all(_.map(migrations, _this5._validateMigrationStructure, _this5));
    }).then(function () {
      return _this5._lockMigrations();
    }).then(function () {
      ...
      return _this5._unlockMigrations();
    })['catch'](this.LockError, function (error) {
      helpers.warn('migrations failed with error: ' + error.message);
      console.log('1111111111111111111111111111111111111111111111', error);
      throw error;
    })['catch'](function (error) {
      helpers.warn('migrations failed with error: ' + error.message);
      console.log('2222222222222222222222222222222222222222222222', error);
      // If the error was not due to a locking issue, then
      // remove the lock. Otherwise, leave it in place to
      // force manual intervention
      return _this5._unlockMigrations().then(function () {
        throw error;
      });
    });
  };
