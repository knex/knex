...
// hack around the fact that 'isParameter' pushes onto bindings when called with a plain
// value, but we want to use it to wrap expressions in () when they are part of a list,
// e.g. group by, order by
function wrapForExpressionList(
  value,
  isParameter,
  builder,
  client,
  ...
  if (typeof value === 'function' || value instanceof QueryBuilder) {
    return wrap(value, true, builder, client, bindingHolder);
  } else {
    return wrap(value, isParameter, builder, client, bindingHolder);
  }
}
