...
class Runner {
  ...
  stream(optionsOrHandler, handlerOrNil) {
    ...
    const cbMediator = { hasConnection: false };
  ...
  async ensureConnection(cb, cbParams, cbMediator) {
    // Use override from a builder if passed
    if (this.builder._connection) {
      return cb(this.builder._connection, this, cbParams, cbMediator);
    }

    if (this.connection) {
      return cb(this.connection, this, cbParams, cbMediator);
    }
    ...
    try {
      return await cb(acquiredConnection, this, cbParams, cbMediator);
    } finally {
