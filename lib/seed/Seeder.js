...
class Seeder {
  ...
  async _listAll(config) {
    ...
    if (this.config.sortDirsSeparately) {
      return seeds.filter(this._filterByLoadExtensions(loadExtensions));
    } else {
      return seeds.filter(this._filterByLoadExtensions(loadExtensions)).sort();
    }
  }

  _filterByLoadExtensions(extensions) {
