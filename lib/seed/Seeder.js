...
const { requireFile } = require('../util/require-interop');
...
class Seeder {
  ...
  async _validateSeedStructure(name) {
    const seed = await requireFile(path.join(this._absoluteConfigDir(), name));
  ...
  async _waterfallBatch(seeds) {
    ...
    for (const seedName of seeds) {
      ...
      const seed = await requireFile(seedPath);
      try {
