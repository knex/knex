...
const { flatten, includes, extend } = require('lodash');
...
class Seeder {
  ...
  async run(config) {
    ...
    if (config && config.specific) {
      const files = all.filter(
        (file) => path.basename(file) === config.specific
      );
      if (files.length === 0) {
        throw new Error(
  ...
  async _runSeeds(seeds) {
    await Promise.all(seeds.map((seed) => this._validateSeedStructure(seed)));
  ...
  async _writeNewSeed(name) {
    ...
    const stubPath = await this._getStubPath();
