...
Seeder.prototype._listAll = _bluebird2.default.method(function (config) {
  ...
  return _bluebird2.default.promisify(_fs2.default.readdir, { context: _fs2.default })(this._absoluteConfigDir()).bind(this).then(seeds => (0, _lodash.filter)(seeds, function (value) {
...
Seeder.prototype._validateSeedStructure = function (name) {
  const seed = require(_path2.default.join(this._absoluteConfigDir(), name));
...
Seeder.prototype._waterfallBatch = function (seeds) {
  ...
  (0, _lodash.each)(seeds, function (seed) {
    const name = _path2.default.join(seedDirectory, seed);
    seed = require(name);

    // Run each seed file.
    current = current.then(() => seed.seed(knex, _bluebird2.default)).then(function () {
      log.push(name);
    });
  });
