...
define(function(require, exports) {
  ...
  var Builder = function(knex) {
    ...
    this.reset();
    _.bindAll(this, '_handleResponse');
  };
  ...
  _.extend(Builder.prototype, Common, {
    ...
    update: function(values) {
      this.values = this._prepValues([values])[0];
    ...
    _handleResponse: function(response) {
      if (this.type === 'select') response = Helpers.skim(response);
      if (this.type === 'insert') response = [response.insertId];
      if (this.type === 'delete' || this.type === 'update') response = response.affectedRows;
      return response;
    },
