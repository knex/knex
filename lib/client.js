const { tracker } = require('./util/connection-tracker.js');
...
class Client extends EventEmitter {
  ...
  initializePool(config = this.config) {
    ...
    const pool = new Pool(tarnPoolConfig);
  ...
  async acquireConnection() {
    ...
    try {
      ...
      debug('acquired connection from pool: %s', tracker.uid(connection));
  ...
  releaseConnection(connection) {
    debug('releasing connection to pool: %s', tracker.uid(connection));
    const didRelease = this.pool.release(connection);

    if (!didRelease) {
      debug('pool refused connection: %s', tracker.uid(connection));
    }
