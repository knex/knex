...
(0, _lodash.assign)(Client.prototype, {
  ...
  query: function query(connection, obj) {
    ...
    return this._query(connection, obj).catch(function(err) {
      err.message =
        _this2._formatQuery(obj.sql, obj.bindings) + ' - ' + err.message;
      _this2.emit(
        'query-error',
        err,
        (0, _lodash.assign)(
          { __knexUid: __knexUid, __knexTxId: __knexTxId },
          obj
        )
      );
      throw err;
  ...
  customWrapIdentifier: function customWrapIdentifier(
    value,
    origImpl,
    queryContext
  ) {
    ...
    return (0, _assign2.default)(poolConfig, {
      create: function create() {
        return _this3.acquireRawConnection().tap(function(connection) {
          ...
          if (poolConfig.afterCreate) {
            return _bluebird2.default.promisify(poolConfig.afterCreate)(
              connection
            );
          }
        });
      ...
      validate: function validate(connection) {
        if (connection.__knex__disposed) {
          _this3.logger.warn(
            'Connection Error: ' + connection.__knex__disposed
          );
          return false;
        }
  ...
  acquireConnection: function acquireConnection() {
    ...
    if (!this.pool) {
      return _bluebird2.default.reject(
        new Error('Unable to acquire a connection')
      );
    }
    ...
    return _bluebird2.default
      ...
      .catch(_tarn.TimeoutError, function() {
        throw new _bluebird2.default.TimeoutError(
