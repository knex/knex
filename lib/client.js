...
class Client extends EventEmitter {
  ...
  positionBindings(sql) {
    // Strip escape backslashes from \? sequences.
    // Dialects that keep ? as their native placeholder (mysql, sqlite) inherit
    // this default.  The raw formatter leaves \? intact so that dialect-specific
    // overrides (pg, mssql, oracle) can decide how to handle escaping.  For
    // dialects that do NOT override, we still need to consume the backslash so
    // the final SQL contains a literal ? rather than \?.
