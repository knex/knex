...
class Client extends EventEmitter {
  constructor(config = {}) {
    ...
    if (this.driverName && config.connection) {
      ...
      if (
        !this.usesInternalTarnPool() &&
        (!config.pool || (config.pool && config.pool.max !== 0))
      ) {
  ...
  setFromInternalTarnPool(connection) {
    throw new Error('Internal Tarn pool is not supported');
  }
  ...
  async acquireConnection() {
    if (!this.usesInternalTarnPool() && !this.pool) {
      throw new Error('Unable to acquire a connection');
    }
    try {
      let connection;
      if (!this.usesInternalTarnPool()) {
        connection = await this.pool.acquire().promise;
      } else {
        connection = await this.acquireRawConnection();
      }

      if (this.usesInternalTarnPool() && !this.pool) {
        this.setFromInternalTarnPool(connection);
      }
