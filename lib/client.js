...
class Client extends EventEmitter {
  ...
  getPoolSettings(poolConfig) {
    const rawPoolConfig = defaults({}, poolConfig, this.poolDefaults());

    POOL_CONFIG_OPTIONS.forEach((option) => {
      if (option in rawPoolConfig) {
        this.logger.warn(
          [
    ...
    const timeouts = [
      ...
      rawPoolConfig.acquireTimeoutMillis,
    ...
    const maxConnectionLifetimeMillis =
      rawPoolConfig.maxConnectionLifetimeMillis;
    const maxConnectionLifetimeJitterMillis =
      rawPoolConfig.maxConnectionLifetimeJitterMillis;
    const userValidate = rawPoolConfig.validate;
    ...
    const generateLifetimeLimit = () => {
      ...
      const jitter =
        ...
        maxConnectionLifetimeJitterMillis > 0
          ...
          : 0;
      return Date.now() + maxConnectionLifetimeMillis + jitter;
    };
    ...
    const userAfterCreate = rawPoolConfig.afterCreate;
      ...
      validate: async (connection) => {
        ...
        if (hasMaxConnectionLifetime) {
          ...
          if (
            ...
            Date.now() > connection.__knexLifetimeLimit
          ) {
            return false;
          }
        }
        ...
        if (
          ...
          if (
            ...
            // If it would still report expired after a refresh, disable it to avoid an infinite refresh/drop loop.
            // Without this guard we could spin forever: checker -> refresh -> still expired -> drop -> repeat.
            // Examples: provider returns already expired data, or the checker is misconfigured to always return true.
            this.connectionConfigExpirationChecker = null;
          }
