...
function Client(config = {}) {
  ...
  if (config.connection && config.connection instanceof Function) {
    ...
    this.connectionSettings = { expired: () => true };
  } else {
...
Object.assign(Client.prototype, {
  ...
  getPoolSettings(poolConfig) {
    ...
    const updatePoolConnectionSettingsFromProvider = () => {
      if (this.connectionConfigProvider === null) {
        // static configuration, nothing to update
        return;
      }
      if (
        this.connectionConfigProvider === undefined &&
        !(this.connectionSettings instanceof Function)
      ) {
        this.connectionConfigProvider = null; // uninitialized, mark as static for future calls
        return;
      }
      // At this point we know a connectionConfigProvider is in use
      if (
