...
class Client extends EventEmitter {
  constructor(config = {}) {
    ...
    if (config.connectionPool) {
      // Native pool mode — skip tarn, delegate to dialect adapter
      this.connectionSettings = {};
      this.connectionConfigExpirationChecker = null;
      if (this.driverName) {
        this.initializeDriver();
        this.initializeNativePool(config);
      }
    } else if (config.connection && config.connection instanceof Function) {
  ...
  initializeNativePool(config = this.config) {
    if (this.pool) {
      this.logger.warn('The pool has already been initialized');
      return;
    }
    this.pool = this.wrapNativePool(config.connectionPool);
    this._ownsPool = false;
  }

  wrapNativePool(_nativePool) {
    throw new Error(
      `The '${this.driverName || 'base'}' dialect does not support the 'connectionPool' option.`
    );
  }
