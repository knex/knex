...
SchemaCompiler_Oracle.prototype.dropTableSuffix = function(options) {
  return options && options.cascade ? ' cascade constraints' : '';
}
...
SchemaCompiler_Oracle.prototype.dropTable = function (tableName, options) {
  this.pushQuery(
...
SchemaCompiler_Oracle.prototype.dropTableIfExists = function(tableName, options) {
  this.pushQuery(utils.wrapSqlWithCatch(
    `drop table ${this.formatter.wrap(tableName)}` +
    this.dropTableSuffix(options)
  , -942));
