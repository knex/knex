...
import KnexBuilderState from '../classes/KnexBuilderState'
...
class Builder extends EventEmitter {
  ...
  constructor(context) {
    ...
    this.__state = new KnexBuilderState(client.log)

    if (client.config && client.config.debug) {
      this.__state.debug(client.config.debug)
    }
  }
  ...
  clone() {
    ...
    cloned.__state = this.__state.clone()
    return cloned;
  }
  ...
  as(column) {
    this.__state.setSingle('as', column)
    return this;
  }

  // Prepends the `schemaName` on `tableName` defined by `.table` and `.join`.
  withSchema(schemaName) {
    this.__state.setSingle('schema', schemaName)
    return this;
  }
  ...
  table(tableName) {
    this.__state.setSingle('table', tableName)
    return this;
  }
  ...
  where(column, operator, value) {
    ...
    if (typeof column === 'object' && !(column instanceof Raw)) return this._objectWhere(column)
  ...
  orWhere() {
    ...
    if (typeof obj === 'object' && typeof obj !== 'function' && !(obj instanceof Raw)) {
  ...
  whereNot() {
    return this._not(true).where(...arguments)
  }
  ...
  union(...args) {
    let [callbacks, wrap] = args
    if (args.length === 1 || (
      args.length === 2 && typeof wrap === 'boolean'
    )) {
      ...
      callbacks = args.slice(0, args.length - 1)
  ...
  offset(value) {
    this.__state.setSingle('offset', value)
    return this;
  }
  ...
  limit(value) {
    ...
    if (isNaN(val)) {
      ...
      this.__state.setSingle('limit', val)
    }
    return this;
  }
  ...
  insert(values, returning) {
    ...
    this.__state.setSingle('insert', values)
    return this;
  }
  ...
  update(values, returning) {
    ...
    this.__state.setSingle('update', obj)
    return this;
  }

  // Sets the returning value for the query.
  returning(returning) {
    this.__state.setSingle('returning', returning)
    return this;
  }
  ...
  columnInfo(column) {
    ...
    this.__state.setSingle('columnInfo', column)
    return this;
  }

  // Set a lock for update constraint.
  forUpdate() {
    this.__state.setSingle('lock', 'forUpdate')
    return this;
  }

  // Set a lock for share constraint.
  forShare() {
    this.__state.setSingle('lock', 'forShare')
    return this;
  }
  ...
  modify(callback, ...rest) {
    callback.apply(this, [this].concat(rest))
    return this;
  }
  ...
  _counter(column, amount, symbol) {
    ...
    this.__state.setSingle('counter', {
      column,
      amount: amt,
      symbol: (symbol || '+')
    })
  ...
  // Internal helpers to maintain backward compatibility
  // for anyone using internal getters / setters pre 0.14

  get _joinFlag() {
    return this.__state.joinFlag
  }
  set _joinFlag(val) {
    this.__state.set('joinFlag', val)
  }

  get _boolFlag() {
    return this.__state.boolFlag
  }
  set _boolFlag(val) {
    this.__state.set('boolFlag', val)
  }

  get _notFlag() {
    return this.__state.notFlag
  }
  set _notFlag(val) {
    this.__state.notFlag = val
  }

  get _statements() {
    return this.__state.statements
  }
  get _single() {
    return this.__state.single
  }

  then(/* onFulfilled, onRejected */) {
    if (!this.__promise) {
      this.__state.toPromise()
