...
describe(dialect + " SchemaBuilder", function() {
  ...
  it('basic create table with column charset', function() {
    tableSql = client.schemaBuilder().createTable('users', function (table) {
      table.increments('id');
      table.string('email').charset('ascii');
    }).toSQL();

    equal(1, tableSql.length);
    expect(tableSql[0].sql).to.equal('create table `users` (`id` int unsigned not null auto_increment primary key, `email` varchar(255) character set \'ascii\')');
  });
