...
module.exports = function(dialect) {
  ...
  describe(dialect + " SchemaBuilder", function() {
    ...
    it('basic create table with column collate', function() {
      ...
      expect(tableSql[0].sql).toEqual('create table `users` (`id` int unsigned not null auto_increment primary key, `email` varchar(255) collate \'utf8_unicode_ci\')');
    });
    ...
    it('test basic create table with charset and collate', function() {
      ...
      expect(tableSql.toSQL()[0].sql).toEqual('create table `users` (`id` int unsigned not null auto_increment primary key, `email` varchar(255)) default character set utf8 collate utf8_unicode_ci');
      expect(tableSql.toQuery()).toEqual('create table `users` (`id` int unsigned not null auto_increment primary key, `email` varchar(255)) default character set utf8 collate utf8_unicode_ci');
    });
    ...
    it('basic create table without charset or collate', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `id` int unsigned not null auto_increment primary key, add `email` varchar(255)');
    });
    ...
    it('drops multiple columns with an array', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` drop `foo`, drop `bar`');
    });
    ...
    it('test drop primary', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` drop primary key');
    });
    ...
    it('test drop unique', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` drop index `users_foo_unique`');
    });
    ...
    it('test drop unique, custom', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` drop index `foo`');
    });
    ...
    it('test drop index', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` drop index `users_foo_index`');
    });
    ...
    it('test drop foreign', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` drop foreign key `users_foo_foreign`');
    });
    ...
    it('test drop foreign, custom', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` drop foreign key `foo`');
    });
    ...
    it('test drop timestamps', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` drop `created_at`, drop `updated_at`');
    });
    ...
    it('test rename table', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('rename table `users` to `foo`');
    });
    ...
    it('test adding primary key', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add primary key `bar`(`foo`)');
    });
    ...
    it('test adding unique key', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add unique `bar`(`foo`)');
    });
    ...
    it('test adding index', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add index `baz`(`foo`, `bar`)');
    });
    ...
    it('test adding incrementing id', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `id` int unsigned not null auto_increment primary key');
    });
    ...
    it('test adding big incrementing id', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `id` bigint unsigned not null auto_increment primary key');
    });
    ...
    it('test adding column after another column', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `name` varchar(255) after `foo`');
    });
    ...
    it('test adding column on the first place', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `first_name` varchar(255) first');
    });
    ...
    it('test adding string', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` varchar(255)');
    });
    ...
    it('uses the varchar column constraint', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` varchar(100)');
    });
    ...
    it('chains notNull and defaultTo', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` varchar(100) not null default \'bar\'');
    });
    ...
    it('allows for raw values in the default field', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` varchar(100) null default CURRENT TIMESTAMP');
    });
    ...
    it('test adding text', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` text');
    });
    ...
    it('test adding big integer', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` bigint');
    });
    ...
    it('test adding integer', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` int');
    });
    ...
    it('test adding medium integer', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` mediumint');
    });
    ...
    it('test adding tiny integer', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` tinyint');
    });
    ...
    it('test adding float', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` float(5, 2)');
    });
    ...
    it('test adding double', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` double');
    });
    ...
    it('test adding double specifying precision', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` double(15, 8)');
    });
    ...
    it('test adding boolean', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` boolean');
    });
    ...
    it('test adding enum', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` enum(\'bar\', \'baz\')');
    });
    ...
    it('test adding date time', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` datetime');
    });
    ...
    it('test adding time', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `foo` time');
    });
    ...
    it('test adding time stamps', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `users` add `created_at` datetime, add `updated_at` datetime');
    });
    ...
    it('is possible to set raw statements in defaultTo, #146', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('create table `default_raw_test` (`created_at` timestamp default CURRENT_TIMESTAMP)');
    });
    ...
    it('allows dropping a unique compound index', function() {
      ...
      equal(1, tableSql.length);
      expect(tableSql[0].sql).toEqual('alter table `composite_key_test` drop index `composite_key_test_column_a_column_b_unique`');
    });
    ...
    it('sets myISAM engine', function() {
      ...
      expect(tableSql[0].sql).toEqual('create table `users` (`username` varchar(255)) engine = myISAM');
    });
    ...
    it('#1430 - .primary & .dropPrimary takes columns and constraintName', function() {
      ...
      expect(tableSql[0].sql).toEqual('alter table `users` add primary key `testconstraintname`(`test1`, `test2`)');

      tableSql = client.schemaBuilder().createTable('users', function(t) {
        t.string('test').primary('testconstraintname');
      }).toSQL();

      expect(tableSql[1].sql).toEqual('alter table `users` add primary key `testconstraintname`(`test`)');
    });
