...
describe('Redshift SchemaBuilder', function() {
  ...
  it('basic alter table', function() {
    ...
    equal(1, tableSql.length);
    expect(tableSql[0].sql).to.equal(
      'alter table "users" add column "id" integer identity(1,1) primary key not null, add column "email" varchar(255)'
    );
  });
  ...
  it('#1430 - .primary & .dropPrimary takes columns and constraintName', function() {
    ...
    equal(1, tableSql.length);
    expect(tableSql[0].sql).to.equal(
      'alter table "users" add column "test1" varchar(255), add column "test2" varchar(255) not null'
    );
    ...
    expect(tableSql[0].sql).to.equal(
      'alter table "users" add column "test1" varchar(255) not null, add column "test2" varchar(255) not null'
    );
    expect(tableSql[1].sql).to.equal(
