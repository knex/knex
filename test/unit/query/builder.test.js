...
function testquery(chain, valuesToCheck, selectedClients) {
  ...
  Object.keys(valuesToCheck).forEach((key) => {
    ...
    const checkValue = valuesToCheck[key];
    expect(checkValue).toBe(sqlString);
  });
}
...
describe('Custom identifier wrapping', () => {
        ...
        ),
      {
        ...
        oracledb: {
          ...
          bindings: (bindings) => {
            ...
            expect(bindings[1]).toBe('taylor');
            expect(bindings[2].toString()).toBe(
              '[object ReturningHelper:id]'
            );
            expect(bindings[3]).toBe('bar');
            expect(bindings[4]).toBe('dayle');
            expect(bindings[5].toString()).toBe(
              '[object ReturningHelper:id]'
            );
          },
        },
        ...
        ),
      {
        ...
        oracledb: {
          ...
          bindings: (bindings) => {
            ...
            expect(bindings[3].toString()).toBe(
              '[object ReturningHelper:name]'
            );
            expect(bindings[4]).toBe('bar');
            expect(bindings[5]).toBe('dayle');
            expect(bindings[6].toString()).toBe(
              '[object ReturningHelper:id]'
            );
            expect(bindings[7].toString()).toBe(
              '[object ReturningHelper:name]'
            );
          },
        },
  ...
  describe('queryContext', () => {
    ...
    it('should return the query context if called with no arguments', () => {
      expect(qb().queryContext({ foo: 'foo' }).queryContext()).toEqual({
        foo: 'foo',
      });
    });
...
describe('QueryBuilder', () => {
    ...
    testsql(qb().into('users').insert([{}], 'id'), {
      ...
      oracledb: {
        ...
        bindings: (bindings) => {
          ...
          expect(bindings[0].toString()).toBe(
            '[object ReturningHelper:id]'
          );
        },
      },
  ...
  //   expect(chain.sql).toBe('update "users" set "email" = ?, "name" = ? from "orders" where "users"."id" = ? and "users"."id" = "orders"."user_id"');
  ...
  it('insert merge with where clause', () => {
    ...
    expect(() => {
      clients.mysql
        ...
        .where('email', 'foo2')
        .toString();
    }).toThrow('onConflict().merge().where() is not supported for mysql');
  });
    ...
    testsql(qb().from('users').insert({ email: 'foo' }, 'id'), {
      ...
      oracledb: {
        ...
        bindings: (bindings) => {
          ...
          expect(bindings[0]).toBe('foo');
          expect(bindings[1].toString()).toBe(
            '[object ReturningHelper:id]'
          );
        },
      },
  ...
  it('#1982 (3) - should throw error when there is subcomments', () => {
    try {
      ...
      expect(error.message).toContain('Cannot include /*, */, ? in comment');
    }
  });
  ...
  it('#4199 - forbids "/*", "*/" and "?" in hint comments', () => {
    ...
    }).toThrow('Hint comment cannot include "/*" or "*/"');
    expect(() => {
      qb().from('testtable').hintComment('hint() */').toString();
    }).toThrow('Hint comment cannot include "/*" or "*/"');
    expect(() => {
      qb().from('testtable').hintComment('hint(?)').toString();
    }).toThrow('Hint comment cannot include "?"');
  });

  it('#4199 - forbids non-strings as hint comments', () => {
    expect(() => {
      qb().from('testtable').hintComment(47).toString();
    }).toThrow('Hint comment must be a string');
    expect(() => {
      qb()
        .from('testtable')
    ...
    qbuilders.forEach(({ builder, undefinedColumns, clientError }) => {
      try {
        ...
        if (clientError && clientError.mysql) {
          expect(error.message).toContain(clientError.mysql);
        } else {
          expect(error.message).toContain(
            'Undefined binding(s) detected when compiling ' +
              builder._method.toUpperCase() +
  ...
  it('Throws error if .first() is called on update', () => {
    try {
      ...
      expect(error.message).toBe('Cannot chain .first() on "update" query');
    }
  });
  ...
  it('Throws error if .first() is called on insert', () => {
    try {
      ...
      expect(error.message).toBe('Cannot chain .first() on "insert" query');
    }
  });
  ...
  it('Throws error if .first() is called on delete', () => {
    try {
      ...
      expect(error.message).toBe('Cannot chain .first() on "del" query');
    }
  });
