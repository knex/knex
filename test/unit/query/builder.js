...
describe('QueryBuilder', function() {
  ...
  it('join with onColumn andOnColumn orOnColumn', () => {
    testsql(
      qb()
        .select({
          id: 'p.ID',
        ...
        .leftJoin({ price: 'wp_postmeta' }, function() {
          this.on('p.id', '=', 'price.post_id')
            .onColumn(function() {
              this.onColumn('price.meta_key', '_regular_price').andOnColumn(
                'price_meta_key',
                '_regular_price'
              );
            })
            .orOnColumn(function() {
              this.onColumn('price_meta.key', '_regular_price');
            });
        }),
      {
        pg: {
    ...
    testsql(
      qb()
        ...
        .leftJoin({ price: 'wp_postmeta' }, (builder) => {
          builder
            .onColumn((q) => {
              q.onColumn('price.meta_key', '_regular_price').andOnColumn(
                'price_meta_key',
                '_regular_price'
              );
            })
            .orOnColumn((q) => {
              q.onColumn('price_meta.key', '_regular_price');
            });
        }),
      {
        pg: {
