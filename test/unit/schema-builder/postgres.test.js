  ...
  describe('check version', function () {
    describe('check version < 9.2', function () {
      ...
      it('client.version', function () {
        expect(knexInstance.client.version).toBe(version);
      });
...
describe('PostgreSQL SchemaBuilder', function () {
  ...
  it('drop table with schema', function () {
    tableSql = client
      ...
      .toSQL();
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('drop table "myschema"."users"');
  });
  ...
  it('drop table if exists with schema', function () {
    tableSql = client
      ...
      .toSQL();
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('drop table if exists "myschema"."users"');
  });
  ...
  it('drop column', function () {
    tableSql = client
      ...
      .toSQL();
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('alter table "users" drop column "foo"');
  });
  ...
  it('drop index, with schema', function () {
    tableSql = client
      ...
      .toSQL();
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('drop index "mySchema"."users_foo_index"');
  });
  ...
  it('throws when adding unique constraint with a predicate', function () {
    expect(() => {
      client
        ...
        .toSQL();
    }).toThrow('postgres cannot create constraint with predicate');
  });
  ...
  it('throws when adding unique index with deferrable set', function () {
    expect(() => {
      client
        ...
        .toSQL();
    }).toThrow('postgres cannot create deferrable index');
  });
  ...
  it('adding enum with useNative and alter', function () {
    ...
    for (let i = 0; i < tableSql.length; i++) {
      expect(tableSql[i].sql).toBe(expectedSql[i]);
    }
  });
  ...
  it('adding multiple useNative enums with some alters', function () {
    tableSql = client
      ...
      .toSQL();
    expect(tableSql.length).toBe(12);
  ...
  it('set empty comment', function () {
    tableSql = client
      ...
      .toSQL();
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('comment on table "user" is \'\'');
  });
  ...
  it("allows creating an extension only if it doesn't exist", function () {
    const sql = client
      ...
      .toSQL();
    expect(sql[0].sql).toBe('create extension if not exists "test"');
  });
  ...
  it('allows dropping an extension only if it exists', function () {
    ...
    expect(sql[0].sql).toBe('drop extension if exists "test"');
  });
  ...
  it('should warn on disallowed method', function () {
    expect(() => {
      tableSql = client
        ...
        .toSQL();
    }).toThrow('Knex only supports engine statement with mysql');
  });
