...
const client = new SQLJS_Client({ client: 'sqljs' });
const {
  parseCreateTable,
  parseCreateIndex,
} = require('../../../lib/dialects/sqljs/schema/internal/parser');
const {
  compileCreateTable,
  compileCreateIndex,
} = require('../../../lib/dialects/sqljs/schema/internal/compiler');

const _ = require('lodash');
const { equal, deepEqual } = require('assert');
const knex = require('../../../knex');

describe('SQLJS SchemaBuilder', function () {
  ...
  it('basic create table without primary key', function () {
    tableSql = client
      .schemaBuilder()
      .createTable('users', function (table) {
        // This make nothing in SQLJS, autoincrement without primary key is a syntax error.
  ...
  it('allows setting a value in the string length, although unused by sqljs', function () {
    tableSql = client
      .schemaBuilder()
      .table('users', function (table) {
  ...
  it('bigincrements works the same as increments for sqljs', function () {
    tableSql = client
      .schemaBuilder()
      .table('users', function (table) {
...
describe('SQLJS parser and compiler', function () {
  const wrap = (v) => `"${v}"`;
