...
describe('MSSQL SchemaBuilder', function () {
  ...
  it('throws when charset and collate are specified', function () {
    expect(() => {
      tableSql = client
        ...
        .toSQL();
    }).toThrow('Knex only supports charset statement with mysql');
  });
  ...
  describe('views', function () {
    ...
    it('rename column of view', function () {
      tableSql = client
        ...
        .toSQL();
      expect(tableSql.length).toBe(1);
      expect(tableSql[0].sql).toBe("exec sp_rename ?, ?, 'COLUMN'");
      expect(tableSql[0].bindings[0]).toBe('[users].oldName');
      expect(tableSql[0].bindings[1]).toBe('newName');
    });
    ...
    it('rename view', function () {
      tableSql = client
        ...
        .toSQL();
      expect(tableSql.length).toBe(1);
      expect(tableSql[0].sql).toBe('exec sp_rename ?, ?');
      expect(tableSql[0].bindings[0]).toBe('old_view');
      expect(tableSql[0].bindings[1]).toBe('new_view');
    });
  ...
  it('test drop column', function () {
    ...
    expect(tableSql.length).toBe(2);
    expect(tableSql[0].sql).toContain(
      `IF @constraint IS NOT NULL EXEC('ALTER TABLE users DROP CONSTRAINT ' + @constraint)`
    );
    expect(tableSql[1].sql).toBe('ALTER TABLE [users] DROP COLUMN [foo]');
  });
  ...
  it('drops multiple columns with an array', function () {
    ...
    expect(tableSql[1].sql).toContain(
      `IF @constraint IS NOT NULL EXEC('ALTER TABLE users DROP CONSTRAINT ' + @constraint)`
    );
  ...
  it('test drop unique, custom', function () {
    ...
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('DROP INDEX [foo] ON [users]');
  });
  ...
  it('test drop index', function () {
    ...
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('DROP INDEX [users_foo_index] ON [users]');
  });
  ...
  it('test rename table', function () {
    ...
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('exec sp_rename ?, ?');
    expect(tableSql[0].bindings[0]).toBe('users');
    expect(tableSql[0].bindings[1]).toBe('foo');
  });
  ...
  it('test rename table with schema', function () {
    ...
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('exec sp_rename ?, ?');
    expect(tableSql[0].bindings[0]).toBe('schema.users');
    expect(tableSql[0].bindings[1]).toBe('foo');
  });
  ...
  it('test has column', function () {
    ...
    expect(tableSql[0].bindings[0]).toBe('foo');
    expect(tableSql[0].bindings[1]).toBe('[users]');
  });
  ...
  it('test has column with schema', function () {
    ...
    expect(tableSql[0].bindings[0]).toBe('foo');
    expect(tableSql[0].bindings[1]).toBe('[schema].[users]');
  });
  ...
  it('throws when adding unique constraint with predicate', function () {
    expect(() => {
      client
        ...
        .toSQL();
    }).toThrow('mssql cannot create constraint with predicate');
  });
  ...
  it('adds foreign key with deferred throw error ', function () {
    ...
    expect(addDeferredConstraint).toThrow('mssql does not support deferrable');
  });
  ...
  it('test adding decimal, no precision', function () {
    expect(() => {
      tableSql = client
        ...
        .toSQL();
    }).toThrow('Specifying no precision on decimal columns is not supported');
  });
  ...
  it('test adding boolean', function () {
    ...
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('ALTER TABLE [users] ADD [foo] bit');
  });
  ...
  it('test adding date time', function () {
    ...
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('ALTER TABLE [users] ADD [foo] datetime2');
  });
  ...
  it('adding binary uuid', function () {
    ...
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('ALTER TABLE [users] ADD [foo] binary(16)');
  });
