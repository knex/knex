...
describe('OracleDb SchemaBuilder', function () {
  ...
  it('adding uuid', function () {
    ...
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('alter table "users" add "foo" char(36)');
  });
  ...
  it('adding binary uuid', function () {
    ...
    expect(tableSql.length).toBe(1);
    expect(tableSql[0].sql).toBe('alter table "users" add "foo" raw(16)');
  });
  ...
  it('should allow using .fn.uuid to create raw statements', function () {
    ...
    expect(helperFunctions.uuid().toQuery()).toBe('(random_uuid())');
  });
