...
// Pre-instantiate knex to ensure all dialect modules are cached in
// Node's require cache before mock-fs replaces the filesystem.
knex({
  client: 'postgres',
  connection: {
    user: 'postgres',
...
describe('Seeder.loadExtensions', function () {
  ...
  it('should include all supported extensions by default', function () {
    return seeder._listAll().then(function (list) {
      expect(normalizePathArray(list)).toEqual(
        normalizePathArray([
...
describe('Seeder._waterfallBatch', function () {
  ...
  it('should throw an error with correct file name', async () => {
    await expect(
      seeder._waterfallBatch([
