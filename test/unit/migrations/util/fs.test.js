...
describe('FS functions', () => {
  describe('stat', () => {
    it('should return stats for a given path.', async () => {
      ...
      expect(result).toBeInstanceOf(fs.Stats);
    });
    ...
    it('should throw an error if the path does not exist.', async () => {
      ...
      await expect(stat(unexistingPath)).rejects.toThrow();
    });
  });
  ...
  describe('writeFile', () => {
    it('should be able to write to the given file path.', async () => {
      ...
      expect(fs.readFileSync(filePath).toString()).toBe('Foo Bar');
    });
  });
  ...
  describe('readdir', () => {
    it('should read a directory and return a list of files under it.', async () => {
      ...
      expect(normalizePathArray(result)).toEqual([
        'testfile1.txt',
        'testfile2.txt',
        'testfile3.txt',
        'testfile4.txt',
