...
describe('cli-config-utils exit()', () => {
  it('Prints Error contents', async () => {
    ...
    expect(stderr).toMatch(/called exit with an Error/);
    // includes stack trace
    expect(stderr).toMatch(/_justexit/);
  });
  it('Handles special formatting for KnexfileRuntimeError (error source)', async () => {
    const { error, stderr } = await exec('knexfile-error-1');
    expect(error.code).toBe(1);
    // includes error message
    expect(stderr).toMatch(/called exit with a KnexfileRuntimeError/);
    // includes detail
    expect(stderr).toMatch(/source error/);
    // includes stack trace
    expect(stderr).toMatch(/_justexit/);
  });
  it('Handles special formatting for KnexfileRuntimeError (non-error source)', async () => {
    ...
    expect(stderr).toMatch(/called exit with a KnexfileRuntimeError/);
    // includes detail
    expect(stderr).toMatch(/config\/path/);
    expect(stderr).toMatch(/some.*object/s);
    // does not include stack trace
    expect(stderr).not.toMatch(/_justexit/);
  });
  it('Prints string contents', async () => {
    const { error, stderr } = await exec('string');
    expect(error.code).toBe(1);
    // includes error message
    expect(stderr).toMatch(/called exit with a string/);
    // does NOT include stack trace
    expect(stderr).not.toMatch(/_justexit/);
  });
  it('Prints stringable contents', async () => {
    ...
    expect(stderr).toMatch(/\{.*called exit with an object.*}/s);
    // does NOT include stack trace
    expect(stderr).not.toMatch(/_justexit/);
  });
  it('Prints non-stringable contents', async () => {
    ...
    expect(error.code).toBe(1);
    // includes stringified value
    expect(stderr).toMatch(/Object: null prototype/);
    expect(stderr).toMatch(/\{.*called exit with a null-prototype object.*}/s);
  });
});
