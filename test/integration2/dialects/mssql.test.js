      ...
      describe(db, () => {
        ...
        describe('column default handling', () => {
          ...
          it('names default constraint', async () => {
            ...
            expect(name).toBe('test_name_default');
          });
          ...
          it("doesn't name default constraint", async () => {
            ...
            expect(name).toMatch(/^DF__test__name__[0-9A-Z]+$/);
          });
        });
        ...
        describe('comment limits', () => {
          ...
          it('supports table comments of max 7500 bytes', async () => {
            // 0 works
            await comment(knex, 'add', byteCount0, tableTarget);

            // 7500 works
            await comment(knex, 'update', byteCount7500, tableTarget);
          ...
          it('supports column comments of max 7500 bytes', async () => {
            // 0 works
            await comment(knex, 'add', byteCount0, columnTarget);

            // 7500 works
            await comment(knex, 'update', byteCount7500, columnTarget);
          ...
          it(`worst-case allows at most ${N} characters with string length ${
            ...
            expect(worstCaseComment).toHaveProperty('length', 2 * N);
            ...
            expect(oneMoreCharIsTooMuch).toHaveProperty('length', 2 * N + 1);
            await expect(
              comment(
                knex,
                'update',
          ...
          it('accepts indexName in options object', async () => {
            ...
            await expect(
              knex
                ...
                .into(tableName)
            ).rejects.toThrow(new RegExp(indexName));
          });
        });
        ...
        describe('comment support', () => {
          ...
          it('attaches table comments', async () => {
            ...
            expect(commentText).toBe(tableComment);
          });
          ...
          it('attaches column comments', async () => {
            ...
            expect(commentText).toBe(columnComment);
          });
          ...
          it('updates table comments', async () => {
            ...
            expect(commentText).toBe(updatedTableComment);
          });
          ...
          it('updates column comments', async () => {
            ...
            expect(commentText).toBe(updatedColumnComment);
          });
          ...
          it('adds column comments when altering and no pre-existing comment', async () => {
            ...
            expect(commentText).toBe(expectedComment);
          });
