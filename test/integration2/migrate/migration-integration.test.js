    ...
    describe(db, () => {
      ...
      describe('knex.migrate', function () {
        it('should not fail on null default for timestamp', async () => {
          try {
            ...
            expect(rows.deleted_at).toBe(null);
          } finally {
        ...
        if (isSQLite(knex)) {
          ...
          it('should not fail alter-table-ignore-case-sensitive', async () => {
            ...
            expect(await knex.schema.hasColumn('Some_Table', 'id')).toBe(true);
            expect(await knex.schema.hasColumn('Some_Table', 'other_id')).toBe(
              true
            );
            expect(await knex.schema.hasColumn('Some_Table', 'name')).toBe(
              true
            );
            // try different case
            expect(await knex.schema.hasColumn('some_table', 'id')).toBe(true);
            ...
            expect(await knex.schema.hasTable('Some_Table')).toBe(false);
            expect(await knex.schema.hasTable('some_table')).toBe(false);
          });
        }
        ...
        it('should create a new migration file with the create method', function () {
          return knex.migrate.make('test').then(function (name) {
            ...
            expect(name.split('_')[0]).toHaveLength(14);
            expect(name.split('_')[1].split('.')[0]).toBe('test');
          });
        });
        ...
        describe('knex.migrate.latest', function () {
          ...
          it('should remove the record in the lock table once finished', function () {
            return knex('knex_migrations_lock')
              .select('*')
              .then(function (data) {
                expect(data[0]).toHaveProperty('is_locked');
                expect(Number.parseInt(data[0].is_locked)).not.toBeTruthy();
              });
          });
          ...
          it('should report failing migration', function () {
            ...
            return migrator
              ...
              .catch(function (error) {
                ...
                expect(migrator).toHaveProperty('_activeMigration');
                expect(migrator._activeMigration).toHaveProperty(
                  'fileName',
          ...
          it('should run the migrations from oldest to newest', function () {
            if (isOracle(knex)) {
              return knex('knex_migrations')
                ...
                .select('*')
                .then(function (data) {
                  expect(path.basename(data[0].name)).toBe(
                    '20131019235242_migration_1.js'
                  );
                  expect(path.basename(data[1].name)).toBe(
          ...
          it('should create all specified tables and columns', function () {
            // Map the table names to promises that evaluate expectations to
          ...
          it('should delete the most recent batch from the migration log', function () {
            return knex.migrate
              ...
              .then(([batchNo, log]) => {
                ...
                expect(log[0]).toContain(batchNo);
                return knex('knex_migrations')
                  .select('*')
                  .then(function (data) {
              ...
              .then(([batchNo, log]) => {
                ...
                fs.readdirSync('test/integration2/migrate/test')
                  ...
                  .forEach((fileName, index) => {
                    expect(fileName).toBe(log[index]);
                  });
        ...
        describe('knex.migrate.list', () => {
          ...
          describe('should list pending and completed migrations', () => {
            it('as an array of arrays of pending and completed migrations', async () => {
              ...
              expect(listedMigrations).toHaveLength(2);
              expect(listedMigrations[0]).toBeInstanceOf(Array);
              expect(listedMigrations[1]).toBeInstanceOf(Array);
            });
      ...
      describe('knex.migrate (transactions disabled)', function () {
        ...
        if (isPostgreSQL(knex)) {
          describe('knex.migrate.latest with specific changelog schema', function () {
            ...
            it('should create changelog in the correct schema without transactions', async function () {
              await knex.migrate
                .latest({
                  directory: 'test/integration2/migrate/test',
                  disableTransactions: true,
              ...
              const data = await knex('testschema.knex_migrations')
                .select('*');
              expect(data.length).toBe(2);
            });

            it('should create changelog in the correct schema with transactions', async function () {
              await knex.migrate
                .latest({
                  directory: 'test/integration2/migrate/test',
      ...
      describe('Test lock row', () => {
        ...
        it('should is_locked value still be 1 if row already exists', async () => {
          ...
          const data = await knex('test_lock').select('*');
          expect(data[0]).toHaveProperty('is_locked');
          expect(Number.parseInt(data[0].is_locked)).toBeTruthy();
        });
      });
    });
  });
