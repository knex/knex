        ...
        it('Allows to specify custom type params', async () => {
          ...
          switch (db) {
            case Db.SQLite:
              ...
              expect(res.find((c) => c.name === colName).type).toBe(
                'varchar(42)'
              );
              break;
            case Db.PgNative:
            case Db.PostgresSQL:
              ...
              expect(res[0].data_type).toBe('character varying');
              expect(res[0].character_maximum_length).toBe(42);
              break;
            case Db.CockroachDB:
              res = await knex
                .select(['data_type', 'character_maximum_length'])
            ...
            case Db.MySQL2:
              res = await knex
                ...
                .from('INFORMATION_SCHEMA.COLUMNS')
                .where({ table_name: tblName, column_name: colName });
              expect(res[0].DATA_TYPE).toBe('varchar');
              expect(res[0].CHARACTER_MAXIMUM_LENGTH).toBe(42);
              break;
          }
        });
