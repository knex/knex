    ...
    describe(db, () => {
        ...
        it('has a dropSchema/dropSchemaIfExists method', async function () {
          ...
          function checkSchemaNotExists(schemaName) {
            knex
              ...
              .then((res) => {
                expect(res[0]).not.toBe(schemaName);
              });
          }
      ...
      describe('createTable', () => {
        describe('like another table', () => {
          ...
          it('copy table', async () => {
            ...
            expect(await knex.schema.hasTable('table_copied')).toBe(true);

            await knex('table_copied')
              .columnInfo()
              .then((res) => {
                expect(Object.keys(res)).toEqual(
                  expect.arrayContaining(['id', 'data'])
                );
              });
          });
                  ...
                  .then((_res) => {
                    ...
                    return knex
                      ...
                      .then((res) => {
                        ...
                        expect(column_name).toBe(expected_column);
                        expect(comment).toBe(expected_comment);
                      });
                  });
              });
          });
        ...
        describe('uuid types - postgres', () => {
          ...
          it('creates a uuid column as primary using fluid syntax', async function () {
            ...
            expect(column_name).toBe(expected_column);
            expect(column_type).toBe(expected_type);
          });
        ...
        it('sets default values with defaultTo', async () => {
          ...
          await knex.schema
            ...
            .then((result) => {
              ...
              if (!isMysql(knex)) {
                expect(result.paragraph).toEqual('Lorem ipsum Qui quis qui in.');
                return;
              }
              if (isPgBased(knex)) {
                expect(result.metadata).toEqual(defaultMetadata);
                expect(result.details).toEqual(defaultDetails);
              } else if (isString(result.metadata)) {
                expect(JSON.parse(result.metadata)).toEqual(defaultMetadata);
              } else {
                expect(result.metadata).toEqual(defaultMetadata);
              }
            });
        ...
        it('rejects setting foreign key where tableName is not typeof === string', () => {
          ...
          expect(() => builder.toSQL()).toThrow(TypeError);
        });
      ...
      describe('table', () => {
        ...
        it('allows alter column syntax', async () => {
          ...
          expect(info.remove_not_null.nullable).toBe(true);
          expect(info.remove_not_null.defaultValue).not.toBe(null);
          expect(info.remove_default.nullable).toBe(false);
          expect(info.remove_default.defaultValue).toBe(null);
          expect(info.remove_default.type).toContain('int');
          ...
          it('should columns order be correctly with after and first', function () {
            ...
            return knex
              ...
              .then((schema) => {
                ...
                // Fields order
                expect(fields[0]).toBe('field_first');
                expect(fields[1]).toBe('field_foo');
                expect(fields[2]).toBe('field_after_foo');
                expect(fields[3]).toBe('field_bar');
                expect(fields[4]).toBe('field_nondefault_increments');
                ...
                // Fields comment
                expect(comments[0]).toBe('First');
                expect(comments[1]).toBe('foo');
                expect(comments[2]).toBe('After');
                expect(comments[3]).toBe('bar');
                expect(comments[4]).toBe('Comment on increment col');
              });
          });
        });
      });
        ...
        describe('without mappers', () => {
          ...
          it('#933 - .renameColumn should not drop null or default value', () => {
            ...
            return knex.transaction((tr) => {
              ...
              return getColInfo()
                .then((colInfo) => {
                  expect(String(colInfo.colnameint.defaultValue)).toContain(
                    '1'
                  );
                  expect(colInfo.colnamestring.defaultValue).toContain('knex');
                  expect(colInfo.colnamestring.nullable).toBe(false);
                ...
                .then((columnInfo) => {
                  ...
                  expect(columnInfo.colnamestringchanged.nullable).toBe(
                    false
                  );
                });
            });
      ...
      describe('dropColumn', () => {
        if (isSQLite(knex)) {
          ...
          describe('when table is created using raw create table', () => {
            beforeEach(async () => {
            ...
            it('drops the column', async () => {
              await dropCol('i0');
              expect(await hasCol('i0')).toBe(false);
              // Constraint i0 should be unaffected:
              expect(await getCreateTableExpr()).toBe(
              ...
              await dropCol('i1');
              expect(await hasCol('i1')).toBe(false);
              ...
              expect(lastColDeletionError.message).toEqual(
        ...
        describe('mssql only', () => {
          ...
          function checkTable(schema, tableName, expected) {
            return knex.schema
              .withSchema(schema)
              .hasTable(tableName)
              .then((exists) => expect(exists).toBe(expected));
          }
          ...
          beforeAll(() => createSchema(testSchemaName));

          afterEach(() =>
            knex.schema.withSchema(testSchemaName).dropTableIfExists('test')
          );
          afterAll(() => knex.schema.raw('DROP SCHEMA ' + testSchemaName));
      ...
      describe('invalid field', () => {
        describe('sqlite3 only', () => {
          ...
          afterAll(() => knex.schema.dropTable(tableName));
