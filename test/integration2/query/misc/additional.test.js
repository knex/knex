    ...
    describe(db, () => {
      ...
      describe('Custom response processing', () => {
        ...
        it('should pass query context to the custom handler', async () => {
          const res = await knex('accounts')
            ...
            .limit(1);
          expect(res.queryContext).toBe('the context');
        });
        ...
        it('should emit error events when a stream query fails', () => {
          const dfd = pDefer();
        ...
        it('should release the connection when a stream query iteration errors', async function () {
          const spy = vi.spyOn(knex.client, 'releaseConnection');
        ...
        it('should process response done through a stream', async () => {
          ...
          await new Promise((done) => {
            ...
            stream.on('finish', () => {
              count++;
              expect(response.callCount).toBe(1);
              done();
            });
          });
          expect(count).toBe(2);
        ...
        it('should pass query context for responses through a stream', () => {
          const dfd = pDefer();
          let response;
          ...
          stream.on('finish', () => {
            expect(response.queryContext).toBe('the context');
            dfd.resolve();
          });
          return dfd.promise;
        });

        it('should process response for each row done through a stream', () => {
          const dfd = pDefer();
      ...
      describe('returning with wrapIdentifier and postProcessResponse` (TODO: fix to work on all possible dialects)', function () {
        ...
        it('should return the correct columns when multiple properties are given to returning', async function () {
          ...
          expect(res).toEqual([{ balance_foo: 123, email_foo: 'foo@bar.com' }]);
        });
      });
      ...
      describe('other operations', () => {
        ...
        it('should allow using the primary table as a raw statement', function () {
          expect(knex(knex.raw('raw_table_name')).toQuery()).toBe(
        ...
        it('should allow using .fn-methods to create raw statements', function () {
          ...
          expect(knex.fn.now().toQuery()).toBe('CURRENT_TIMESTAMP');
          expect(knex.fn.now(6).toQuery()).toBe('CURRENT_TIMESTAMP(6)');
        });
        ...
        it('should allow using .fn.uuid to create raw statements', function () {
          ...
          if (isRedshift()) {
            expect(() => knex.fn.uuid().toQuery()).toThrow(
              `${knex.client.driverName} does not have a uuid function`
            );
          } else {
            expect(knex.fn.uuid().toQuery()).toBe(
        ...
        it('should insert binary uuid and retrieve it with not ordered uuid data', async () => {
          ...
          expect(expectedUuid).toBe(originalUuid);
        });
        ...
        it('should allow renaming a column', async function () {
          ...
          if (isMysql(knex)) {
            ...
            expect(aboutCol).toEqual(newAboutCol);
          }

          const countAfterRename = (await knex.count('*').from('accounts'))[0][
            countColumn
          ];
          expect(countAfterRename).toBe(count);
        ...
        it('should allow dropping a column', async function () {
          ...
          await knex
            ...
            .then(function (resp) {
              expect(_.keys(resp).sort()).toEqual([
                'about',
                'balance',
                'created_at',
                'email',
            ...
            .then(function (resp) {
              expect(resp[0][countColumn]).toBe(count);
            });
      ...
      describe('timeouts', () => {
        it('.timeout() should throw TimeoutError', async function () {
          ...
          try {
            ...
            // Await the rest of the 1 second sleep so we don't see the command in subsequent tests
        ...
        it('.timeout(ms, {cancel: true}) should throw TimeoutError and cancel slow query', async function () {
          ...
          try {
            ...
            expect(sleepProcess).toBe(undefined);
          }
        });
        ...
        it('.timeout(ms, {cancel: true}) should cancel slow query even if connection pool is exhausted', async function () {
          ...
          try {
            ...
            expect(processesBeforeTimeout).toContain(query.toString());
            ...
            expect(processesAfterTimeout).not.toContain(query.toString());
          } finally {
      ...
      describe('Events', () => {
        it('Event: query-response', async function () {
          ...
          const onQueryResponse = function (response, obj, builder) {
            ...
            expect(obj.__knexUid).toBeTypeOf('string');
            expect(obj.__knexQueryUid).toBeTypeOf('string');
            expect(builder).toBeTypeOf('object');
          };
        ...
        it('Event: preserves listeners on a copy with user params', async function () {
          ...
          expect(Object.keys(knex._events).length).toBe(1);
          expect(Object.keys(knexCopy._events).length).toBe(1);
          knex.removeListener('query-response', onQueryResponse);
          expect(Object.keys(knex._events).length).toBe(0);
          expect(queryCount).toBe(4);
        });
        ...
        it('Event: query-error', async function () {
          ...
          try {
            ...
            expect(true).toBe(false); //Should not be resolved
          } catch (err) {
            knex.removeListener('query-error', onQueryErrorKnex);
            knex.removeListener('query-error', onQueryErrorBuilder);
            expect(queryCountBuilder).toBe(1);
            expect(queryCountKnex).toBe(1);
          }
        });
        ...
        it('Event: start', async function () {
          ...
          expect(row.last_name).toBe('Start event test');
        });
        ...
        it("Event 'query' should not emit native sql string", function () {
          ...
          builder.on('query', function (obj) {
            ...
            if (native !== sql) {
              expect(obj.sql).not.toBe(builder.toSQL().toNative().sql);
              expect(obj.sql).toBe(builder.toSQL().sql);
            }
          });
      ...
      describe('async stack traces', function () {
        ...
        it('should capture stack trace on raw query', async () => {
          try {
            ...
            expect(err.stack.split('\n')[2]).toMatch(/at Object\.raw \(/); // the index 2 might need adjustment if the code is refactored
        ...
        it('should capture stack trace on schema builder', async () => {
          try {
            ...
            expect(err.stack.split('\n')[1]).toMatch(/client\.schemaBuilder/); // the index 1 might need adjustment if the code is refactored
            expect(typeof err.originalStack).toBe('string');
          }
        });
      });
      ...
      describe('misc', () => {
        ...
        it('should allow destroying the pool with knex.destroy', async function () {
          ...
          const spy = vi.spyOn(knexDb.client.pool, 'destroy');
          await knexDb.destroy();
          expect(spy).toHaveBeenCalledTimes(1);
          expect(knexDb.client.pool).toBe(undefined);
          await knexDb.destroy();
          expect(spy).toHaveBeenCalledTimes(1);
        });
        ...
        it('should allow initialize the pool with knex.initialize', async function () {
          ...
          expect(knexDb.client.pool).toBe(undefined);
          knexDb.initialize();
          expect(knexDb.client.pool.destroyed).toBe(false);
          const waitForDestroy = knexDb.destroy();
          expect(knexDb.client.pool.destroyed).toBe(true);
        ...
        it('should not leak passwords when logged', async function () {
          ...
          expect(passwordMatches).toBeNull();
        });
      });
    });
  });
