    ...
    describe(db, () => {
      ...
      describe('where like', async function () {
        ...
        it('finds data using andWhereLike', async () => {
          ...
          expect(result[3].email).toBe('test4@example.com');
          expect(result[4].email).toBe('test5@example.com');
          expect(result[5].email).toBe('test6@example.com');
          expect(result[6].email).toBe('test7@example.com');
          expect(result[7].email).toBe('test8@example.com');
        });
      ...
      it('Retains array bindings, #228', function () {
        ...
        expect(raw.toSQL().bindings).toEqual(
          knex.client.prepBindings(expected1)
        );
        expect(raw2.toSQL().bindings).toEqual(
