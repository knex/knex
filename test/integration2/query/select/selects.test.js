...
describe('Selects', function () {
  getAllDbs().forEach((db) => {
    describe(db, () => {
      ...
      it('#4335 - should throw an error when negative offset provided', function () {
        expect(() => {
          knex.from('accounts').limit(20).offset(-20);
        }).toThrow('A non-negative integer must be provided to offset.');
      });
      ...
      it('#4199 - adheres to hint comments', async function () {
        ...
        const baseQuery = knex('accounts')
          ...
          .limit(2);
        await expect(baseQuery.clone()).resolves.toHaveLength(2);
        await expect(
          baseQuery.clone().hintComment('max_execution_time(10)')
        ).rejects.toMatchObject(expectedErrors[knex.client.driverName]);
      });
      ...
      it('returns a stream if not passed a function', async function () {
        if (isPgNative(knex)) {
          return;
        }
      ...
      it('allows you to stream with mysql dialect options', async function () {
        ...
        rows.forEach((row) => {
          ['first_name', 'last_name', 'email'].forEach((field) =>
            expect(row[field]).toBe(row[field].toUpperCase())
          );
        });
      });
      ...
      it('emits error on the stream, if not passed a function, and query fails', async function () {
        ...
        await new Promise((resolve, reject) => {
          ...
          stream.on('end', () => reject(new Error('Expected error')));
        });
      });

      it('emits error if not passed a function and the query has wrong bindings', async function () {
      ...
      it('throws errors on the asCallback if uncaught in the last block', async function () {
      ...
      it('#1276 - Dates NULL should be returned as NULL, not as new Date(null)', async function () {
        ...
        expect(rows[0].dateTimeCol).toBe(null);
        expect(rows[0].timeStampCol).toBe(null);
        expect(rows[0].dateCol).toBe(null);
        expect(rows[0].timeCol).toBe(null);
      });
      ...
      it('knex.ref() as column in .select()', async function () {
        ...
        const row = await knex('accounts')
          ...
          .first();
        expect(String(row.userid)).toBe(String(result[0].id));
      });
      ...
      it('select for update locks selected row', async function () {
        ...
        expect('Second query should have timed out').toBe(false);
      });
      ...
      it('select for share prevents updating in other transaction', async function () {
        ...
        if (isSQLite(db) || isOracle(db) || isCockroachDB(db)) {
          return;
        }
        await knex('test_default_table').insert({
          string: 'making sure there is a row to lock',
      ...
      it('forUpdate().noWait() should throw an error immediately when a row is locked', async function () {
        ...
        try {
          ...
          if (isPostgreSQL(knex)) {
            expect(err.message).toContain('could not obtain lock on row');
            ...
            // expect(err.message).toContain('Lock wait timeout exceeded');
          } else {
      ...
      it('select from subquery', async function () {
        ...
        try {
          ...
          expect([
            ...
            'pg',
            'pgnative',
          ]).toContain(knex.client.driverName);
