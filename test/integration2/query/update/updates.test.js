    ...
    describe(db, () => {
        ...
        async function runTest() {
          return Promise.all(
            res.map(async (origRow) => {
              ...
              expect(updatedRow[0].balance).toBe(654);
              ...
              expect(updatedRow2[0].balance).toBe(origRow.balance);
            })
          );
        }
      ...
      it('should increment a float value', async function () {
        ...
        const accounts2 = await knex('accounts')
          .select('balance')
          .where('id', accountId1);
        expect(accounts[0].balance + 22.53).toBeCloseTo(
          accounts2[0].balance,
          0.001
        );
      });
      ...
      it('should decrement a float value', async function () {
        ...
        const accounts2 = await knex('accounts')
          .select('balance')
          .where('id', accountId1);
        expect(accounts[0].balance - 10.29).toBeCloseTo(
          accounts2[0].balance,
          0.001
        );
      });
      ...
      it('with update query', async function () {
        ...
        const results = await knex('accounts')
          .from('accounts')
          .where('email', '=', 'test1@example.com');
        expect(results[0].last_name).toBe('olivier');
      });
