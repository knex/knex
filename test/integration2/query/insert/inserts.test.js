...
describe('Inserts', function () {
  getAllDbs().forEach((db) => {
    describe(db, () => {
      ...
      it('should allow for using the `asCallback` interface', function () {
        const dfd = pDefer();
        knex('test_table_two')
          .insert(
            [
      ...
      it('should throw an error if the array passed in is empty', async function () {
        await expect(knex('account').insert([])).rejects.toThrow(
          Error,
          'The query is empty'
        );
      });
      ...
      it('should take an array of columns to return in oracle or postgres', async function () {
        ...
        if (isRedshift(knex)) {
          return expect(rows).toBe(1);
        }
        expect(rows.length).toBe(1);
        if (isPostgreSQL(knex)) {
          expect(_.keys(rows[0]).length).toBe(2);
          expect(rows[0].account_id).toBe(insertData.account_id);
          expect(rows[0].details).toBe(insertData.details);
        }
      });
      ...
      it('should allow a * for returning in postgres and oracle', async function () {
        ...
        expect(rows.length).toBe(1);
        if (isPgBased(knex)) {
          expect(_.keys(rows[0]).length).toBe(4);
          assertNumber(knex, rows[0].account_id, insertData.account_id);
          expect(rows[0].details).toBe(insertData.details);
          expect(rows[0].status).toBe(insertData.status);
        }
      });
        ...
        it('#757 - knex.batchInsert(tableName, bulk, chunkSize)', async function () {
          ...
          if (isPostgreSQL(knex) || isOracle(knex)) {
            result.forEach(function (item) {
              expect(item.Col1).toBe(fiftyLengthString);
              expect(item.Col2).toBe(fiftyLengthString);
            });
          }

          const selectResult = await knex('BatchInsert').select();
          const count = selectResult.length;
          expect(count).toBe(amountOfItems);
        });
      ...
      it('should validate batchInsert batchSize parameter', async function () {
        ...
        try {
          ...
          expect(error.message).toBe('Invalid chunkSize: null');
          try {
            knex.batchInsert('test', [], 0);
          } catch (error) {
            expect(error.message).toBe('Invalid chunkSize: 0');
            try {
              //Also faulty
              knex.batchInsert('test', [], 'still no good');
            } catch (error) {
              expect(error.message).toBe('Invalid chunkSize: still no good');
              return true;
            }
          }
        }
      ...
      it('should replace undefined keys in multi insert with DEFAULT', async function () {
        ...
        expect(results[1].about).toBe(null);
      ...
      it('updates specified columns with insert value when inserting a duplicate key to unique column and merge with update columns is specified', async function () {
        ...
        const rows = await knex('upsert_tests')
          ...
          .select();
        expect(rows.length).toBe(1);
        expect(rows[0].name).toBe('SHOULD BE USED');
      });
      ...
      describe('text array', () => {
        ...
        it('#5365 should insert string array to text ARRAY column', async function () {
          ...
          await knex(tableName)
            ...
            .then((result) => {
              expect(result.length).toBe(1);
              expect(result[0].content).toEqual(stringArrayContent);
            });
        });
