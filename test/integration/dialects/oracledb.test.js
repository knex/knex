...
const config = require('#test/knexfile.js');

describe.runIf(config.oracledb)('Oracle', () => {
  describe('Compiler', () => {
  ...
  describe('OracleDb externalAuth', function () {
    ...
    beforeAll(function () {
      spy = vi.spyOn(knexInstance.client.driver, 'getConnection');
    });
    ...
    it('externalAuth and connectString should be sent to the getConnection', function () {
      ...
      expect(spy).toHaveBeenCalledWith(connectionWithExternalAuth);
    });
  });
    ...
    describe('with fetchAsString parameter ', function () {
      ...
      it('on clob', function () {
        return knexClient
          ...
          .then(function (result) {
            expect(result[0]).toBeTruthy();
            expect(result[0].field).toBe('LONG CONTENT');
          });
      });
    ...
    describe('without fetchAsString parameter', function () {
      ...
      it('on float', function () {
        return knexClient
          ...
          .then(function (result) {
            expect(result[0]).toBeTruthy();
            expect(typeof result[0].field).not.toBe('string');
          });
      });
      ...
      it('on blob', async () => {
        ...
        expect(result[0]).toBeTruthy();
        expect(result[0].field.toString('hex')).toBe(
      ...
      it('on raw', function () {
        return knexClient
          ...
          .then(function (result) {
            expect(result[0]).toBeTruthy();
            expect(result[0].field).toBeInstanceOf(Buffer);
          });
      });
  ...
  describe('OracleDb unit tests', function () {
    ...
    it('disposes the connection on connection error', async function () {
      ...
      vi.spyOn(knexClient.client, 'acquireConnection').mockImplementation(
        async () => {
          const conn = await acquireConnection.call(knexClient.client);
          conn.release();
          spy = vi.spyOn(conn, 'close');
          return conn;
        }
      );
    ...
    it('clears the connection from the pool on disconnect during commit', async function () {
      ...
      const spy = vi.spyOn(knexClient.client, 'releaseConnection');
