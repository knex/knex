...
const logger = require('../logger-vitest');
...
getAllDbs().forEach((db) => {
  describe(`${db} - Insert with Triggers`, () => {
    let knex;
    beforeAll(() => {
    ...
    describe('Trigger Specific Tests', function () {
      ...
      it('#4152 Should allow returns with inserts on tables with triggers', async function () {
        ...
        await knex.transaction(async function () {
          ...
          expect(Number.isFinite(Number.parseInt(insertResults))).toBe(true);
      ...
      it('should allow a * for returning in postgres and oracle', function () {
        ...
        return knex('test_table_two')
          ...
          .then(function (rows) {
            expect(rows.length).toBe(1);
            if (isPgBased(knex)) {
              expect(_.keys(rows[0]).length).toBe(5);
              expect(rows[0].account_id).toBe(insertData.account_id);
              expect(rows[0].details).toBe(insertData.details);
      ...
      it('should replace undefined keys in multi insert with DEFAULT', function () {
        ...
        return knex('accounts')
          ...
          .then(function (results) {
            expect(results[0].logins).toBe(1);
