      ...
      wantedBindings.forEach(function (wantedBinding, index) {
        if (typeof wantedBinding === 'function') {
          expect(
            ...
            'binding checker function failed got: ' + gotBindings
          ).toBe(true);
        } else {
          expect(gotBindings[index]).toEqual(wantedBinding);
        }
      });
      expect(gotBindings.length).toBe(wantedBindings.length);
    } else {
        ...
        qb.then = function () {
          ...
          promise = promise.then(function (resp) {
            if (typeof returnval === 'function') {
              expect(!!returnval(resp)).toBe(true);
            } else {
              try {
